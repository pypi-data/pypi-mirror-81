...
def tif_to_df(infile, drop_nodata=True, split_size=5000):
    ...
    if nrows * ncols > split_size * split_size:
        ...
        for tif_file in tif_files:
            print('start', tif_file)
            parquet_file = os.path.splitext(tif_file)[0] + '.snappy.parquet'
            tif_to_df(tif_file, drop_nodata, split_size).to_parquet(parquet_file, engine='pyarrow', compression='snappy', index=False)
