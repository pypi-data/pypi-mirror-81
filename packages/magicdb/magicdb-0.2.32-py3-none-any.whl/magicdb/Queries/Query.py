...
class Query:
    ...
    @contextmanager
    def safe_span(self, query_type):
        if not SHOULD_USE_SPAN:
            print("should not use span...")
            yield
        else:
            full_label = (
                f"{query_type}-{self.get_path_of_query()}-{str(self.query_inputs)}"
            )
            with span(full_label):
                print("there is span!")
                yield
    ...
    def get(self, id=None, create=False, validate_db_data=True, **kwargs):
        ...
        with self.safe_span("get"):
            doc = self.firebase_query.get()
