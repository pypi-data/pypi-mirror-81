...
class Kproxy(object):
    def __init__(self):
        ...
        self.url = "https://dps.kdlapi.com/api/getdps/?orderid=929071356227231&num=15&pt=1&format=json&sep=1"
    ...
    def val(self, proxy):
        self.starttime_val = time.time()
        url = f"https://dps.kdlapi.com/api/getdpsvalidtime?orderid=929071356227231&signature=xjnjhv99ag3abk80hbnji139nc032p04&proxy={proxy}"
    ...
    def val_all(self):
        self.starttime_val = time.time()
        sql = "select proxy from kuaidailiproxy where val_stat=1"
    ...
    def run(self):
        ...
        while True:
            if int(time.time() - self.starttime) > 20:
                self.get_proxy()
            if int(time.time() - self.starttime_val) > 5:
                self.val_all()
            time.sleep(5)
