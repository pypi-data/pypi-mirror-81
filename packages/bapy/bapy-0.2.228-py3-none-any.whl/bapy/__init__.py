...
class Frame:
    ...
    def __init__(self, index: Int = index, ctx: Int = ctx):
        ...
        if back := getattr(frame, 'f_back', None):
            ...
            self.back_function: CodeType = self.back_code.co_name
...
class MongoBase:
    ...
    """
    _db_name: Text = None
    uri_name: Literal['pen'] = 'pen'
    uri: Text = dataclasses.field(init=False)
    Filters: Any = collections.namedtuple('Filters', 'col_names')
    filters: Filters = Filters({"name": {"$regex": r"^(?!system\.|.*file|.*glob|.*invalid)"}})

    def __post_init__(self):
        if self.uri_name == 'pen':
            env = Env().file
            username = env('USERNAME', os.environ.get('USERNAME', None))
            password = env('NFERX_ATLAS_PASSWORD', os.environ.get('NFERX_ATLAS_PASSWORD', None))
            self.uri = f'mongodb+srv://{username}:{password}@{self.uri_name}.ydo6l.mongodb.net/' \
                       f'{self.uri_name}?retryWrites=true&w=majority'
        else:
            self.uri = 'localhost'

    @property
    def client(self) -> UnionMongo:
        return MotorClient(self.uri) if aioloop() else MongoClient(self.uri)

    @property
    def col_names(self) -> List:
        return self.db.list_collection_names(filter=self.filters.col_names)

    @property
    async def col_names_async(self) -> List:
        return await self.db.list_collection_names(filter=self.filters.col_names)

    @property
    def cols_iter(self) -> List:
        return [self.db.get_collection(item) for item in self.col_names]

    @property
    async def cols_iter_async(self) -> List:
        return [self.db.get_collection(item) for item in await self.col_names_async]

    @property
    def cols_count_estimated(self) -> Dict:
        return {item.name: item.estimated_document_count() for item in self.cols_iter}

    @property
    async def cols_count_estimated_async(self) -> Dict:
        return {item.name: await item.estimated_document_count() for item in await self.cols_iter_async}

    def cols_distinct(self, field: Text = '_id') -> Dict:
        return {item.name: item.distinct(field) for item in self.cols_iter}

    async def cols_distinct_async(self, field: Text = '_id') -> Dict:
        return {item.name: await item.distinct(field) for item in await self.cols_iter_async}
...
@dataclasses.dataclass
class Mongo:
    """Mongo Collection Helper Class."""
    _col_name: Text = None
    col_name_default: Text = 'nferx'
    _base: MongoBase = None

    @property
    def base(self) -> MongoBase:
        return self._base if self._base else MongoBase()

    @base.setter
    def base(self, value: Text):
        self._base = MongoBase(value)

    @property
    def col(self) -> Any:
        return self.base.db.get_collection(self.col_name)

    @property
    def col_default(self) -> Any:
        return self.base.db.get_collection(self.col_name_default)
    ...
    def distinct(self, field: Text = '_id') -> List:
        return self.col.distinct(field)

    async def distinct_async(self, field: Text = '_id') -> List:
        return await self.col.distinct(field)


@dataclasses.dataclass
class MongoMal(Count):
    """MongoDB Helper Class."""
    _collection: Any = None
    _collection_name: Any = None
    db: Any = dataclasses.field(init=False)
    _db_name: Text = dataclasses.field(init=False)
    _id: Union[List, Any] = None
    _port: Int = 27017
    ...
    def collection_names(self):
        f = {"name": {"$regex": r"^(?!system\.)"}}
        return self.db.list_collection_names(filter=f)
...
def paths(file: Text) -> Path:
    ...
    try:
        ...
        if env.file.bool(upper_prefix('TRACE_MALLOC', prefix=prefix), None):
            import tracemalloc
            tracemalloc.start()
        if env.file.bool(upper_prefix('ASYNCIO_DEBUG', prefix=prefix), None):
            env.environ = {'PYTHONASYNCIODEBUG': 1}
...
class Url(furl.furl):
    ...
    """
    Domain: Any = collections.namedtuple('Domains', 'nferx nference nferxops')
    domain: Domain = Domain('nferx.com', 'nference.net', 'nferxops.net')
    bind: Int = None
