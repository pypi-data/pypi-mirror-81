...
searchInput.addEventListener("input",function(){setHiddenOfElement(searchClear,(searchInput.value.length===0))})};var showScreenshotsButtonContainer=document.querySelector('.rm-remote-app-privacy-button-container')
if(showScreenshotsButtonContainer!==null){var showScreenshotsAnchor=showScreenshotsButtonContainer.querySelector('a')
showScreenshotsAnchor.addEventListener('click',function(event){event.preventDefault()
...
for(var i=0;i<screenshotsData.length;i++){addScreenshot(screenshotsContainer,screenshotsData[i].value)}
...
function addScreenshot(screenshotsContainer,url){var container=document.createElement('div')
