...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        if not self._session_id:
            ...
            self._ssm_client = self._get_boto_client("ssm")
            self.start_ssm_session()
        return self

    def _get_instance_id(self):
        cmd = self.get_option("host_command").replace("$HOST", self.host)
        return to_text(subprocess.check_output(shlex.split(cmd)).strip())
    ...
    def _start_process(self, cmd):
        ...
        self._poll_stderr.register(self._process.stderr, select.POLLIN)
    ...
    def _get_boto_client(self, service):
        return boto3.client(service, region_name=self.get_option("region"))
