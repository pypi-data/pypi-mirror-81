...
from ufotest.util import execute_command, get_repository_name_from_url
...
def install_pcitools(path:str, verbose=True):
    ...
    folder_path = os.path.join(path, get_repository_name_from_url(git_url))
    exit_code = install_generic_cmake(
        path,
        git_url,
        verbose,
        {'CMAKE_INSTALL_PREFIX': '/usr'}
