...
def test_from_hex_string():
    assert utils.from_hex_string('00') == 0
    assert utils.from_hex_string('0F') == 15
    assert utils.from_hex_string('10') == 16
    assert utils.from_hex_string('16') == 22

    assert utils.from_hex_string('00:0F:10:16') == [0, 15, 16, 22]
    assert utils.from_hex_string('00:0F:10:16') == [0x00, 0x0F, 0x10, 0x16]
