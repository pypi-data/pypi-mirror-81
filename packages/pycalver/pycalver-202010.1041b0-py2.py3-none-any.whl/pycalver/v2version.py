...
def _parse_version_info(field_values):
    ...
    vinfo = version.V2VersionInfo(year_y=year_y, year_g=year_g, quarter=
        quarter, month=month, dom=dom, doy=doy, week_w=week_w, week_u=
        week_u, week_v=week_v, major=major, minor=minor, patch=patch, num=
        num, bid=bid, tag=tag, pytag=pytag, inc0=inc0, inc1=inc1)
    return vinfo


def parse_version_info(version_str, raw_pattern='vYYYY0M.BUILD[-RELEASE]'):
...
def is_valid(version_str, raw_pattern='vYYYY0M.BUILD[-RELEASE]'):
...
def incr(old_version, raw_pattern='vYYYY0M.BUILD[-RELEASE]', **kwargs):
    ...
    cur_vinfo = _incr_numeric(raw_pattern, old_vinfo, cur_vinfo, major=
        major, minor=minor, patch=patch, tag=tag, release_num=release_num)
