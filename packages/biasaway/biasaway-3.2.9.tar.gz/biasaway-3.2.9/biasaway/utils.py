...
def make_gc_plot(fg_gc, bg_gc, plot_filename):
    ...
    import pandas as pd
    plt.figure()
    df = pd.concat([pd.DataFrame({'input': fg_gc}),
                   pd.DataFrame({'generated': bg_gc})], axis=1,
                   ignore_index=True)
    df.columns = ["input", "generated"]
    sns.displot(df, kind=kind)
    plt.xlabel("%GC")
    plt.ylabel(ylab)
...
#    from numpy import hstack
    ...
    plt.figure()
    import pandas as pd
    df = pd.concat([pd.Series(fg_len), pd.Series(bg_len)], axis=1,
                   ignore_index=True)
    df.columns = ["input", "generated"]
    sns.displot(df, kind=kind)
    plt.xlabel('length')
