...
from azureml._restclient.models import LocalRunGetNextTaskInput, MiroProxyInputDto
        ...
        try:
            if self.automl_settings.spark_context:
                ...
                try:
                    ...
                    raise ConfigException._with_error(
                        AzureMLError.create(
                            LocalLegacyDependencyMissing, target="local", module_name=e.name, inner_exception=e)
                    ) from e
    ...
    def _get_task(self, previous_fit_outputs, is_sparse=None):
        ...
        with logging_utilities.log_activity(logger=logger, activity_name=TelemetryConstants.GET_PIPELINE_NAME):
            try:
                ...
                previous_pipeline_results = None  # type: Optional[MiroProxyInputDto]
                if previous_fit_outputs is not None:
                    # TODO: VSO-562043 - Add training_percents as well
                    previous_pipeline_results = MiroProxyInputDto(
                ...
                return self._jasmine_client.get_next_task(self.parent_run_id, local_run_get_next_task_input)
            except (AzureMLException, AzureMLServiceException):
                raise
            except Exception as e:
                logging_utilities.log_traceback(e, logger)
