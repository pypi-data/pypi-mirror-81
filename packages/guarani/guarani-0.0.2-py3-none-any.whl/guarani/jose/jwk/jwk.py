...
from guarani.jose.jwk.algorithms import JWKAlgorithm, OCTKey, RSAKey, ECKey
...
class JsonWebKey(dict):
            ...
            def validate_attr(self, key: dict) -> None:
                if attr := key.get("attr"):
                    if not isinstance(attr, str):
                        raise InvalidKeyError("The provided attr is invalid.")
    ...
    def generate(cls, algorithm: str, option: Any = None, **params) -> JsonWebKey:
        ...
        :raises UnsupportedAlgorithmError: Unsupported algorithm. 😒
    ...
    def parse(
        ...
        **options,
        ...
        :raises UnsupportedAlgorithmError: Algorithm not supported.
        :raises UnsupportedParsingMethodError: Method not supported (alg).
        :raises InvalidKeyError: The raw key type is different from the class' (alg).
    ...
    def validate_kty(self, key: dict) -> None:
        ...
        :raises InvalidKeyError: No key type was provided.
        :raises UnsupportedAlgorithmError: The provided key type is unsupported.
        """

        if not (kty := key.get("kty")):
            raise InvalidKeyError('Missing parameter "kty".')
    ...
    def validate_use_and_key_ops(self, key: dict) -> None:
        ...
        :raises InvalidKeyError: Not a valid key use or operations.
        :raises InvalidUseKeyOpsError: The `use` and `key_ops` do not match.
        """

        if use := key.get("use"):
            if not isinstance(use, str):
                raise InvalidKeyError("The provided use is invalid.")

        if key_ops := key.get("key_ops"):
            if not isinstance(key_ops, list):
                raise InvalidKeyError("The provided key_ops is invalid.")
    ...
    def validate_alg(self, key: dict) -> None:
        ...
        :raises InvalidKeyError: The provided JWS or JWE algorithm is invalid.
        """

        if alg := key.get("alg"):
            if not alg or not isinstance(alg, str):
                raise InvalidKeyError('Invalid parameter "alg".')
    ...
    def validate_kid(self, key: dict) -> None:
        ...
        :raises InvalidKeyError: The provided key ID is invalid.
        """

        if kid := key.get("kid"):
            if not kid or not isinstance(kid, str):
                raise InvalidKeyError('Invalid parameter "kid".')
