                        ...
                        try:
                            ...
                            for i in range(0, len(methods)):
                                ...
                                if not 'key' in methods[i]:
                                    methods[i]['key']= self._serviceConfig['key'].upper()+"."+serviceClass.__name__.upper()+"_" +func_name
                                else:
                                    if not methods[i]['key']:
                                        methods[i]['key']= self._serviceConfig['key'].upper()+"."+serviceClass.__name__.upper()+"_" +func_name
                                    else:
                                        if len(methods[i]['key'].split('.'))==1:
                                            methods[i]['key']= self._serviceConfig['key'].upper()+"."+func_name
                                        else:
                                            logging.error("%s key can not contain (.) ." % methods[i]['key'])
