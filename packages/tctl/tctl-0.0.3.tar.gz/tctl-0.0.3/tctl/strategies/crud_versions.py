...
def strategy_deploy(options):
    ...
    langs_keys = [l.replace(" ", "").replace("-", "").lower() for l in langs]
    lang = options.get("lang", [])[0]

    if lang in langs_keys and lang not in ["python3", "python2", "node"]:
        click.echo("-----------------------------")
        click.echo(click.style("PLEASE NOTE:", fg="red"))
        click.echo("Currently, only Python and Node are supported by the backtester.")
        click.echo("-----------------------------\n")
