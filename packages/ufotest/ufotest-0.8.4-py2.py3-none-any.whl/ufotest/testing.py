...
class TestRunner(object):
    ...
    def __init__(self, config: Config = Config()):
        ...
        self.folder_path = os.path.join(
            ...
            self.start_time.strftime(self.config.get_test_folder_format())
        )
...
class CombinedTestResult(AbstractTestResult):
    ...
    def to_json(self) -> str:
        ...
        for result in self.test_results:
            combined.update(json.loads(result.to_json()))
    ...
    def to_json(self) -> str:
        return json.dumps({
            ...
            'overview': {
                ...
                'errors': self.error_count,
                'coverage': self.success_ratio
            },
            'details': dict((name, json.loads(result.to_json())) for name, result in self.results.items())
        }, indent=4)
