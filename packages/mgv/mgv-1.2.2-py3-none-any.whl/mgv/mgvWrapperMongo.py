...
def getObjects(graph):
    ...
    # test if the graph is a template or not
    if graph.path[-1] == '*template*':
        pid = db[project_name].find_one({'code': 'GraphTemplate', 'name': graph.getName(), 'parent': graph.pattern.uuid},
                                        {'_id': 1})['_id']
        g = db[project_name].find_one({'code': 'Graph', 'name': graph.getName(), 'parent': pid}, {'template_uuid': 1})
    # test if the graph object contains a uuid
    elif graph.uuid:
        g = db[project_name].find_one({'code': 'Graph', '_id': graph.uuid}, {'template_uuid': 1})
    else:
    ...
    if g is None:
        return None
    # if template is not create in graph
    if 'template_uuid' not in g.keys():
        return None
