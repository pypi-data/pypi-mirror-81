...
import sys
from pymvngit.utils import Utils, Colors
import locale
...
class PrettyTable:
    ...
    def printHeader(self):
        ...
        for header in self.header:
            ...
            value               = Colors.BLUE + header + Colors.RESET + space_remaining + Colors.RESET
    ...
    def printMeInAscii(self, tableArgs=None):
        ...
        for row in self.content:
            ...
            for col in row:
                ...
                space_remaining     = ' '.ljust(number_remain_space,' ')
                value               = Colors.GREEN + " " + str(col) + space_remaining + Colors.RESET
                if self.is_number(col):
                   ...
                   value = space_remaining + Colors.GREEN + vlrNumber + Colors.RESET + " "
                vlr_row = vlr_row + "|" + value
                idx_col+=1
            vlr_row += "| "   
            ...
            if originalHighLightValue:
               ...
               vlr_row = highLightValue_backGroundColor + Colors.BLACK + vlr_row.strip() + Colors.RESET
               vlr_row = vlr_row.replace(originalHighLightValue, Colors.IBLUE + originalHighLightValue + Colors.BLACK )
            ...
            if not tableArgs.filterValue or tableArgs.filterValue.strip() == "":
               ...
               if originalFilterValue:
                  vlr_row = vlr_row.replace(originalFilterValue, Colors.IBLUE + originalFilterValue + Colors.GREEN ) 
                  self.printedTable += vlr_row + "\n" 
