...
class Users(viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        email, password, discourse_password, username, full_name = (
            data.get('email', ''),
            data.get('password', ''),
            data.get('discourse_password', ''),
            data.get('username', ''),
            data.get('full_name', ''),
        )
        ...
        result = self.create_discourse_user(email, discourse_password, username, full_name)
        if not result:
            result = {}
    ...
    def create_discourse_user(self, email: str, password: str, username: str, full_name: str):
        email, username, _, _ = self.normalize_user_info(email, username, full_name)
        server = 'https://discuss.remo.ai'
        headers = {"Accept": "application/json; charset=utf-8"}
        r = requests.get(f'{server}/users/hp.json', headers=headers).json()
