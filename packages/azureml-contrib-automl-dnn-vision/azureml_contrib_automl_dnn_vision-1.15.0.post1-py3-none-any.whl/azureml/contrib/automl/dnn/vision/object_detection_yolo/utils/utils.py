...
def load_image(path, img_size, augment):
    ...
    """
    # TODO RK: make it consistent with the other reading method, and use the ignore_errors flag
...
def load_mosaic(index, image_urls, img_size, mosaic_border, labels, augment, affine_settings):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w) = load_image(image_urls[index], img_size, augment)
...
def get_image(index, image_urls, img_size, all_labels, settings, is_train):
    ...
    if mosaic:
        # Load mosaic
        img, labels = load_mosaic(index, image_urls, img_size, mosaic_border, all_labels, augment, settings)

    else:
        # Load image
        img, (h0, w0), (h, w) = load_image(image_urls[index], img_size, augment)
