...
from torusnet.TorusISlipRouterRTL import TorusISlipRouterRTL as TorusRouterRTL
...
class TestHarness( Component ):
  ...
  def construct( s,
    ...
    def up_dut():
      for i in range( 5 ):
        s.dut.recv[i].msg @= s.src_adapters[i].send.msg
        s.dut.recv[i].en  @= s.src_adapters[i].send.en
        s.src_adapters[i].send.yum[0] @= s.dut.recv[i].yum[0]
        s.src_adapters[i].send.yum[1] @= s.dut.recv[i].yum[1]

        s.sink_adapters[i].recv.msg @= s.dut.send[i].msg
        s.sink_adapters[i].recv.en  @= s.dut.send[i].en
        s.dut.send[i].yum[0] @= s.sink_adapters[i].recv.yum[0]
        s.dut.send[i].yum[1] @= s.sink_adapters[i].recv.yum[1]
  ...
  def line_trace( s ):
    # return f'{s.srcs[4].line_trace()}'
    return f'{s.src_adapters[4].line_trace()}'
