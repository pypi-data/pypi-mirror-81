...
from .score import calculate_mi, calculate_pc
...
def get_mutual_information(graph, df):
    """
    ...
    len_df = len(df)
    max_samples = 5000
    n_samples = min(len_df, max_samples)

    df_samples = df.sample(n_samples)

    ret = []

    data_output = df_samples[graph[-1].name]
    activations = _get_activations(graph, df_samples)
    for n in range(len(graph)):
        ret.append(calculate_mi([activations[n]], data_output))

    return ret

def get_pearson_correlations(graph, df):
    """
