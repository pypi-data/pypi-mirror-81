...
- Fixes
    - Removed the configuration of tkinter as the matplotlib backend from the top level code. This has
      caused a crash of all CLI commands when running on a headless linux, which does not have an x-server
      running
    - The archive folder names now actually use the template from the config file
    - Changed the default date format order of the archive folders in such a way that they are now ordered
      correctly by name
    - install functions for the dependency repositories now return the exit code of the operation
- Added
    - Every test run now additionally created a JSON file with the most important details about that test
      run.
    - The "install" command now creates a file "ufotest_install.json", which contains the results of the
      installation process in JSON format
