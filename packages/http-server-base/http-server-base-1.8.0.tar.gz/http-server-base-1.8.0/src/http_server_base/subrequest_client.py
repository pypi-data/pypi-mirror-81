...
class SubrequestClient(RequestLoggerClient, ErrorMatcher):
    ...
    async def _fetch__fetch_request(self, request: HttpSubrequest, intercept_connection_errors: bool) -> HttpSubrequestResponse:
        ...
        return response
    async def _fetch__check_response__content_type_matching(self, response: HttpSubrequestResponse, *, expected_content_type: Optional[str]) -> Optional[Dict[str, Any]]:
        if (expected_content_type is None):
            return None
        elif (expected_content_type == '*/*'):
            return None
        elif ('Content-Type' not in response.headers):
            return dict(message="Missing 'Content-Type' header in the response", expected_mime_type=expected_content_type)
        else:
            ...
            return dict(message=f"Missing '{expected_content_type}' in 'Content-Type' headers of the response ({','.join(response.headers.get_list('Content-Type'))})", expected_mime_type=expected_content_type)
    async def _fetch__check_response(self, response: HttpSubrequestResponse, *, expected_codes: Collection[int], expected_content_type: Optional[str]):
        ...
        if (alert is None and expected_content_type):
            alert = await self._fetch__check_response__content_type_matching(response, expected_content_type=expected_content_type)
    ...
    async def fetch_binary_data(self, request: Union[str, HTTPRequest], **kwargs) -> Tuple[HttpSubrequestResponse, bytes]:
        ...
        if (not alert and base_error):
            try:
                ...
                alert = dict(message=resp_json.get('reason') or resp_json.get('error') or (resp_json.get('errors') and resp_json['errors'][0]) or resp_json.get('message'), server_response=resp_json)
