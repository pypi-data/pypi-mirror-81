...
def get_celeb_name_from_id(result_list, celeb_mapping_dict, dist_threshold=0.9):
...
def get_celeb_prediction(img, ann_index, celeb_mapping_dict):
	# ann_index = AnnoyIndex(2048, 'angular')
	...
	if encs is not None:
		for index, enc in enumerate(encs):
			...
			dist_threshold = 0.9
			celeb_count_dict = get_celeb_name_from_id(results, celeb_mapping_dict, dist_threshold)
