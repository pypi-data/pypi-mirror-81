...
class AKHintWidget(BoxLayout):
    ...
    def _show_hover(self, instance, pos):
        if self.collide_point(pos[0], pos[1]):
            ...
            Clock.schedule_once(
                lambda x: self._change_opacity(1),
                self.show_delay)
            self._state = 'open'
        else:
    ...
    def _set_hintwidget_pos(self, pos):
        ...
        if mag_right >= image_right:
            ...
            if self.hintwidget_pos in ['tr', 'br']:
                ...
                self._hintwidget_x = pos[0] - \
                    self.offset_x - self.hintwidget_width

        if mag_top >= image_top:
            self._hintwidget_y = pos[1] - \
                self.offset_y - self.hintwidget_height
    ...
    def _change_opacity(self, opacity):
        if not self._opac_anim_started:
            ...
            Clock.schedule_once(
                lambda x: self._allow_opac_animation(),
    ...
    def on_touch_down(self, touch):
        ...
        if self.show_mode == 'touch' and self.collide_point(pos[0], pos[1]):
            ...
            Clock.schedule_once(
                lambda x: self._change_opacity(opac),
                self.show_delay)
