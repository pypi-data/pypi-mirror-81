...
class RunBase(object):
    ...
    def compute_lineage(self):
        ...
        for p in allentries:
            ...
            for d in dependencies:
                ...
                isdict = isinstance(objects,dict)
                for o in objects:

                    detail = objects[o] if isdict else o
                    label = o

                    if type_ in ['file']:
                        label = label if isdict else os.path.basename(label)
                        node = "{} ({}, {})".format(label, framename, transform)
                        text = "Path: {}".format(detail)
                    elif type_ in ['dataframe', 'dataframes']:
                        type_ = "dataframe"
                        node_transform = last.get(label, transform)
                        node = "{} ({})".format(label, node_transform)
                        text = "Dataframe: {}".format(detail)
                    elif type_ in ['database', 'db', 'databases']:
                        type_ = "database"
                        node_transform = last.get(label, transform)
                        if "/" in label and (not isdict):
                            node = "{} ({})".format(os.path.basename(label), transform)
                        else:
                            node = "{} ({})".format(label, transform)
                        text = "DB: {}".format(detail)
                    elif type_ in ['api', 'url']:
                        type_ = "url"
                        node_transform = last.get(label, transform)
                        if "/" in label and (not isdict):
                            node = "{} (API, {})".format(os.path.basename(label), transform)
                        else:
                            node = "{} (API, {})".format(label, transform)
                        text = "URL: {}".format(detail)
    ...
    def compute_compliance_metadata(self):
        ...
        for framename in metadata['details']:
            ...
            # => Now collect all the information in a single dict
            frames[framename] = {
                'framename': framename,
                'frametype': frametype,
...
class RunManagerBase(object):
    ...
    def load(self, offset=0, page=50):
        """
        Load the runs

        Args:

             full (bool): Load all runs (default: False)
        ...
        for outputdir in outputdirs:
            ...
            # If we have not seen the outputdir, then include it.
...
class PipelineConfig(object):
    ...
    def set_params(self):
        ...
        for attr in ['customer_root', 'data_root',
                     'output']:
