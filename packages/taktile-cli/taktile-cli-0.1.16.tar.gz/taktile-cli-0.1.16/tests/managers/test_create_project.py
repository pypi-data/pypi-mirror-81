...
def test_create_in_existing_directory(monkeypatch, tmpdir):
    ...
    for file in TEMPLATE_PROJECT_FILES:
        ...
        assert os.path.exists(os.path.join(tmpdir.dirpath(), "test_project_dir", basename))
    assert os.path.exists(os.path.join(tmpdir.dirpath(), "test_project_dir", "tktl.yaml"))
