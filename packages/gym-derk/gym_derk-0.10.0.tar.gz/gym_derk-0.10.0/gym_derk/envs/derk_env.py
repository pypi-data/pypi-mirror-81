...
class DerkEnv(gym.Env):
  ...
  * ``mode="connected"``: Connects two or more environments together.
    By default the current environment is connected any "server" mode environment running on the same machine.
  ...
  Args:
    ...
    connected_envs: Environments to connect to in "connected" mode. See below for details.
  ...
  The ``connected_envs`` argument takes list of dicts with the following format:
  ``{ uri: str, regions: [{ side: str, start_arena: int, n_arenas: int }] }``, where
  ``uri`` specifies a running DerkEnv to connect to, and regions define which arenas and sides
  that environment will control.
  ``side`` can be ``'home'``, ``'away'`` or ``'both'``. ``start_arena`` and ``n_arenas`` can be ommitted
  to run the environment on all arenas. ``connected_envs`` defaults to the following in connected mode:
  ...
  Attributes:
    ...
    action_keys: Enum with all action outputs (len=5)
    observation_keys: Enum with all observation outputs
    total_reward: Accumulated rewards over an episode
    episode_stats: Stats for the last episode

  """
  def __init__(self,
      mode: str=False, server_port: int=None, server_host: str=None, connected_envs: List[Dict]=None,
      app_host: str=None,
    ...
    self.server_uri = 'ws://' + self.server_host + ':' + str(self.server_port)
    self.connected_envs = connected_envs
    if self.connected_envs is None:
      if self.mode == 'connected':
        self.connected_envs = [{ 'uri': self.server_uri, 'regions': [{ 'sides': 'home' }] }, { 'uri': 'ws://127.0.0.1:8789', 'regions': [{ 'sides': 'away' }] }]
      else:
        self.connected_envs = [{ 'uri': self.server_uri, 'regions': [{ 'sides': 'both' }] }]
    ...
    self.action_keys = enum.Enum('Actions', ['MoveX', 'Rotate', 'ChaseFocus', 'CastingSlot', 'ChangeFocus'], start=0)
    ...
    self.observation_space = gym.spaces.Box(low=-1, high=1, shape=[len(self.observation_keys)])
  ...
  async def _connect_environments(self):
    await self.page.evaluate('''(envs, nArenas, inWorker) => window.derk.connectToEnvironments(envs, nArenas, inWorker)''', self.connected_envs, self.n_arenas, self.web_socket_worker)
  ...
  async def _handle_websocket(self, websocket, path):
    ...
    initMsg = json.loads(await websocket.recv())
    self.n_teams = initMsg['nTeams']
    self.observation_keys = enum.Enum('Observations', initMsg['observationKeys'], start=0)
  ...
  def _decode_observations(self, observations):
    ...
    return obs.reshape((int(len(self.observation_keys) / 4), -1, 4)).swapaxes(0, 1).reshape((-1, len(self.observation_keys)))
