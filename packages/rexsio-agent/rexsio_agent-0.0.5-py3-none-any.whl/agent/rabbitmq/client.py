...
class RabbitMqClient(object):
    ...
    @staticmethod
    def get_instance():
        if RabbitMqClient._instance is None:
...
class ReconnectingRabbitMqClient(object):
    ...
    def __init__(self):
        ...
        self._client = RabbitMqClient.get_instance()
        self.thread = None
        self.stop_consuming_event = threading.Event()

    def start_consuming_messages(self):
        logger.info("Starting consuming messages on RabbitMQ")
        self.thread = threading.Thread(
            target=self._run, args=(self.stop_consuming_event,)
        )
        self.thread.start()
    ...
    def _maybe_reconnect(self):
        if self._client.should_reconnect:
            self._client.stop()
