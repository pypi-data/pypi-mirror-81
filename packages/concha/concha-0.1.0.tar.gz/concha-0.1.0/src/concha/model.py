...
from tensorflow.keras.layers import Dense, Activation, Multiply, concatenate, Input, Lambda, Dropout, BatchNormalization
...
class DeepLearningEstimator():
    ...
    def create_mlp_regression_model(num_units, num_layers, dropout, l2_penalty):
        ...
        Args:
            ...
            dropout (float): Rate of dropout between 0.0 and 1.0 for a dropout layer placed after every dense layer.
    ...
    def get_output_at_layer(model, X, layer_index=0):
        ...
        """
        get_n_layer_output = K.function([model.layers[0].input],
    ...
    def prepare_features(features_df, categorical_feature_cols=None):
        ...
        if categorical_feature_cols is None:
            ...
            categorical_feature_cols = [col for col in categorical_feature_cols if col not in ['tmin', 'tmax']]

        continuous_feature_cols = [col for col in features_df.columns if col not in categorical_feature_cols]


        feature_converter = ColumnTransformer([
            ('cat_converted', OneHotEncoder(sparse=False, handle_unknown='ignore'), categorical_feature_cols),
...
class ProfitMaximizer(BaseEstimator, RegressorMixin, DeepLearningEstimator):
    ...
    def __init__(
        ...
        self.regr_loss_ = self.create_maximize_profit_regression_loss_function(self.marginal_cost, self.unit_sale_price)
        ...
        self.optimal_rounding_fn_ = self.make_rounding_direction_function(self.batch_size, self.batch_cost, self.unit_sale_price)
    ...
    def fit(self, X, y=None, validation_data=None, reset_model=False):
        ...
        X_conv, self.X_converter_ = self.prepare_features(X, categorical_feature_cols=self.categorical_feature_cols)
        ...
        # If each fit needs to be from scratch, recompile the model
        if reset_model:
            self.regression_model_.compile(loss=self.regr_loss_, optimizer='adam')
        ...
        regr_prediction= self.y_converter_.inverse_transform(self.regression_model_.predict(X_conv))
        ...
        batch_fraction = regr_prediction/self.batch_size - np.floor(regr_prediction/self.batch_size) - 0.5
    ...
    def predict(self, X):
        ...
        rounded_down_output = np.floor(regr_prediction/self.batch_size)*self.batch_size
        
        # if discretizer predicts more profit from rounding up, round up to next batch.
        batch_rounded_prediction = np.where(disc_prediction <= 0, rounded_down_output, rounded_down_output + self.batch_size)
    ...
    def make_rounding_direction_function(batch_size, batch_cost, unit_sale_price):
        ...
        def make_target(y_true, y_regr):
            ...
            lower_profit = K.minimum(y_true, lower_output)*np.array([unit_sale_price]) - lower_output / batch_size *np.array([batch_cost])
            upper_profit = K.minimum(y_true, upper_output)*np.array([unit_sale_price]) - upper_output / batch_size *np.array([batch_cost])
...
class QuantileRegressor(BaseEstimator, RegressorMixin, DeepLearningEstimator):
    ...
    def __init__(
        ...
        self.regr_loss_ = self.create_quantile_regression_loss_function(self.demand_quantile)   
    ...
    def fit(self, X, y=None, validation_data=None, reset_model=False):
        ...
        if validation_data is not None:
            X_conv_test = self.X_converter_.transform(validation_data[0]).astype(np.float32)
            y_conv_test = self.y_converter_.transform(validation_data[1]).astype(np.float32)
            validation_data = (X_conv_test, y_conv_test)
            earlystopping_monitor='val_loss'
        else:
            earlystopping_monitor='loss'
    ...
    def create_quantile_regression_loss_function(quantile):
        ...
        Args:
            quantile (float): Proportion between 0.0 and 1.0 of demand probability density to predict. 0.5 predicts the median (half the time demand is lower, half the time it's higher).
        ...
        @tf.function
        def loss(y_true, y_pred):
            """Quantile loss function. The higher the quantile, the more the model penalizes predicting too low,
...
class MeanWeekPart():
    ...
    def predict(self, X):
        y = X['day_of_week'].apply(lambda dow: self.weekend_mean if dow in self.weekends else self.weekday_mean)
