...
class CSGenerator:
    ...
    def _register_customized_class(self, cls):
        if cls in self._classes or cls in self._enums or cls in self._without_annos:
            return
        if issubclass(cls, Enum):
            self._enums.add(cls)
        else:
            annos = _get_all_annotations(cls)
            if len(annos) == 0:
                self._without_annos.add(cls)
