...
def _stats(df: pd.DataFrame):
    ...
    by_error = df.error.count()
    content = "No processing error detected."
    if by_error > 0:
        content = f"{by_error} errors detected"
...
def _resources(df):
    ...
    last = (
        df[df.end > (time.time() - duration)].groupby(by=by).generation.count().nlargest()
    )

    # print some columns
    left = Section(
        df.groupby(by=by).generation.count().nlargest().to_string(),
