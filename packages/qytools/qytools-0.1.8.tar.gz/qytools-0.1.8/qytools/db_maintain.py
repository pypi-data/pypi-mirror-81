...
class Dbtools:
	...
	def split_data(dbpath, tablename, index_col, df_split, insert=False, insertmode='new'):
		...
		if 'code' in index_col.split(','):
			...
			code_fields = str(list(df_split['date'].unique())).replace('[', '(').replace(']', ')')
			str_code = str_code + code_fields
		else:
			str_code = ''
		if 'date' in index_col.split(','):
			maxdt = f" AND date >= '{df_split['date'].max()}'"
			mindt = f" AND date <= '{df_split['date'].min()}'"
			if insert:
				if insertmode == 'new':
					maxdate = pd.read_sql(
			...
			maxdt = f" AND date >= '{df_split['datetime'].max()}'"
			mindt = f" AND date <= '{df_split['datetime'].min()}'"
			if insert:
				if insertmode == 'new':
					maxdate = pd.read_sql(
...
if __name__ == '__main__':
	factor = Dbtools()
	factor.creat_dbtable(dbpath='D:/test.sqlite3', tablename='xxxxx', index_col='code,datetime', key_col='code')
	# db_read = DBReader('')
	# # 准备入库的df
