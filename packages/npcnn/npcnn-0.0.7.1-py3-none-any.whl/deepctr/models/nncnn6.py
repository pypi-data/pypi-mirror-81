...
def NNCNN6(linear_feature_columns, dnn_feature_columns, 
         conv_kernel_width=(5, 5, 3), conv_filters=(64, 64, 64), mlp_ch_filters=((64, 64), (64, 64), (64, 64)), 
         mlp_feat_filters=((22, 22), (22, 22), (22, 22)), pooling_width=(3, 3), ave_pooling_width=8, ave_stride=1,
    ...
    for i in range(1, l + 1):
        ...
        nin_result = tf.keras.layers.Conv2D(filters=c_filters[0], kernel_size=(1, 1), strides=(1, 1), padding='same',
                                             activation='relu', use_bias=True, )(conv_result)
        nin_result = tf.keras.layers.Conv2D(filters=c_filters[1], kernel_size=(1, 1), strides=(1, 1), padding='same',
                                             activation='relu', use_bias=True, )(nin_result)
        
        tr_result = tf.transpose(conv_result, [0, 3, 2, 1])
        nin_result = tf.keras.layers.Conv2D(filters=f_filters[0], kernel_size=(1, 1), strides=(1, 1), padding='same',
                                             activation='relu', use_bias=True, )(tr_result)
        nin_result = tf.keras.layers.Conv2D(filters=f_filters[1], kernel_size=(1, 1), strides=(1, 1), padding='same',
                                             activation='relu', use_bias=True, )(nin_result)
        nin_result = tf.transpose(nin_result, [0, 3, 2, 1])
        if i < l:
