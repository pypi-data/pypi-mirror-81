...
from exclock.rumps_app import RumpsApp
    ...
    try:
        ...
        if PLATFORM == 'Darwin':
            RumpsApp(
                clock_title='⏰ Exclock ⏰',
                message=message,
                ring_filename=ring_sound_filename,
                maxsec=spend_sec).run()
        else:
            notify_all_term(
                title='⏰ Exclock ⏰', message=message, spend_sec=spend_sec, is_first=False)
