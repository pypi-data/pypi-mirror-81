...
class RMQClient:
    ...
    async def connect(self, config=RabbitConfig):
        ...
        self.queue: aio_pika.Queue = await self.channel.declare_queue(self.queue_name, auto_delete=True, durable=False)
    ...
    async def send_request(self, path,  body: str = None, json: dict = None,
                           headers=None) -> GenericResponse:
        ...
        # print('publish on', path, '@', message_id)
        try:
            await ex.publish(
                aio_pika.Message(
                    body.encode(),
