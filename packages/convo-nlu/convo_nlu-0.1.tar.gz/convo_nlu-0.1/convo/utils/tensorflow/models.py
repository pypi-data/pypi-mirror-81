...
from convo.utils.common import is_logging_disabled
from convo.utils.tensorflow.model_data import ConvoModelData, FeatureSignature
from convo.utils.tensorflow.constants import SEQUENCE, TENSORBOARD_LOG_LEVEL
...
class ConvoModel(tf.keras.models.Model):
    ...
    def build_for_predict(
        self, predict_data: ConvoModelData, eager: bool = False
    ) -> None:
    ...
    def predict(self, predict_data: ConvoModelData) -> Dict[Text, tf.Tensor]:
    ...
    @classmethod
    def load(
        cls, model_file_name: Text, model_data_example: ConvoModelData, *args, **kwargs
    ) -> "ConvoModel":
    ...
    def _get_tf_train_functions(
        self, eager: bool, model_data: ConvoModelData, batch_strategy: Text
    ...
    def _get_tf_evaluation_functions(
        self, eager: bool, evaluation_model_data: Optional[ConvoModelData]
