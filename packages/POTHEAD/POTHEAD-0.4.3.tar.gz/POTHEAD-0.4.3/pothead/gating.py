...
def wait_for_idle_cpus(required, *, max_concurrent=None, fixed_delay=0.2, inertia=0.7):
    ...
    def wait_for_slot(halt):
        ...
        cpu_poll_interval = max(CPU_IDLE_POLL_MIN_INTERVAL, min(cpu_poll_interval.value, CPU_IDLE_POLL_MAX_INTERVAL))
        while workers.count > 0 and not halt():
            if (
                ...
                cpu_poll_interval.update((time() - start_time) / CPU_IDLE_POLL_DIVISOR)
                break
