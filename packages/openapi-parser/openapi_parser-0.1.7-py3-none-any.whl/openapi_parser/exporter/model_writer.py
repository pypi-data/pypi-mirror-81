...
from openapi_parser.parser import OpenApiParser, find_filters, EnumFilter
...
class ModelWriter(ClassWriter, HeaderWriter, FooterWriter):
    ...
    def dump_model(self, parser: OpenApiParser) -> Iterator[str]:
        for path, mdl in parser.loaded_objects.items():
            if (isinstance(mdl, ModelSchema)):
                for enum in find_filters(mdl.filter, EnumFilter):
                    yield from self.dump_enum(mdl, enum)
    ...
    # noinspection PyOverloads
    @overload
    def write_model_file(self, parser: OpenApiParser, *, file: TextIO) -> None:
        pass
    @writer
