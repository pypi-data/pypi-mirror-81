    ...
    steps = [
        ...
        },
        {
            ...
            "cmd": [
                ...
                "enrichpkg bootstrap -c transform-simple -p transforms/persona.py",
                "# Will create a python module",
                "enrichpkg bootstrap -c transform-simple -p transforms/persona",
                "# Will create a python package",
                "enrichpkg bootstrap -c transform-package -p transforms/persona",
        ...
        {
            "cmd": [
                "enrichpkg bootstrap -c pipeline-conf -p pipelines/conf/persona.py"
            ],
...
@click.pass_context
def _testdata_download(ctx, dataset, command, src, dst, start, end):
    """
    Download dataset for a specified range

    For command, src and dst use 'show'
    """

    package.testdata_action("download",
                            ctx.obj['spec'],
                            ctx.obj['capture'],
                            ctx.obj['context'],
                            name=dataset,
