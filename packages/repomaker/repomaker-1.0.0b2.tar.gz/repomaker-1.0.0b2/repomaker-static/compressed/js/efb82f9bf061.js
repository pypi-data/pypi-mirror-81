;var holders=[document.getElementById('rm-dnd-holder--screenshots'),document.getElementById('rm-dnd-holder--apks'),document.getElementById('rm-dnd-holder-2--apks'),document.getElementById('rm-dnd-holder--feature-graphic'),]
function uploadFiles(element,files){var type=element.id.split("--").pop()
var formData=new FormData()
for(var i=0;i<files.length;i++){if((type=='screenshots'||type=='feature-graphic')&&!isImage(files[i])){showError(element,gettext('You can only upload images here.'))
return}
...
request.setRequestHeader('RM-Background-Type',type)
request.onloadstart=uploadStarted(element,files)
request.upload.onprogress=function(event){updateProgress(element,event)}
request.onreadystatechange=function(){if(request.readyState===4){uploadFinished(request,element,type,files)}}
request.send(formData)}
if('draggable'in document.createElement('span')&&!!window.FormData){showDndTexts()
holders.forEach(function(holder){if(holder!=null){holder.ondragover=function(){return onDragOver(this)}
holder.ondragleave=function(){return onDragLeave(this)}
holder.ondrop=function(event){return onDrop(this,event)}
holder.onchange=function(){return onChange(this)}}})}
function onDragOver(element){element.classList.add('rm-dnd-hover')
return false}
function onDragLeave(element){element.classList.remove('rm-dnd-hover')
return false}
function onDrop(element,event){fromHoverToLoading(element.classList)
uploadFiles(element,event.dataTransfer.files)
return false}
function onChange(element){fileElement=element.getElementsByTagName('input')[0]
if(fileElement==null)return false;fromHoverToLoading(element.classList)
uploadFiles(element,fileElement.files)
fileElement.value=null
return false}
function fromHoverToLoading(classList){classList.remove('rm-dnd-hover')
classList.add('rm-dnd-loading')}
function uploadStarted(element,files){var loadingElement=document.getElementById(element.id+'--loading')
element.hidden=true
loadingElement.hidden=false
var loadingElementTitle=document.getElementById(loadingElement.id+'-title')
loadingElementTitle.textContent=interpolate(ngettext('Uploading %s file...','Uploading %s files...',files.length),[files.length])}
function updateProgress(element,event){if(event.lengthComputable){var progress=(event.loaded/event.total)*100
var progressBar=document.querySelector('.rm-dnd-progress')
progressBar.MaterialProgress.setProgress(progress)}}
function uploadFinished(request,element,type,files){if(type==='screenshots'&&request.status===200){var response=JSON.parse(request.responseText)
addScreenshots(element,response)}
else if(type==='feature-graphic'&&request.status===200){var response=JSON.parse(request.responseText)
setFeatureGraphic(element,response)}
else if(type==='apks'&&request.status===200){var response=JSON.parse(request.responseText)
addApks(element,response)}
else if(request.status===204){location.reload()}
else{showError(element,request.responseText)}}
function addScreenshots(dndField,response){var screenshotsContainer=dndField.parentElement
var repo=response['repo']
var app=response['app']
var screenshots=response['screenshots']
for(var i=0;i<screenshots.length;i++){var id=screenshots[i]['id']
...
function setFeatureGraphic(dndField,response){var featureGraphicContainer=dndField.parentElement
var repo=response['repo']
var app=response['app']
var featureGraphicUrl=response['feature-graphic']
var featureGraphicImg=document.getElementById('rm-app-feature-graphic-img')
if(featureGraphicImg===null){featureGraphicImg=document.createElement('img')
...
else{featureGraphicImg.src=featureGraphicUrl
...
function addApks(dndField,response){var apksContainer=dndField.parentElement
var appVersionList=apksContainer.querySelector('.rm-app-versions-list')
if(appVersionList===null){appVersionList=document.createElement('ul')
...
for(var i=0;i<apks.length;i++){var id=apks[i]['id']
...
resetDndField(dndField)}
function showDndTexts(){var elements=document.getElementsByClassName('rm-dng-text')
for(var i=0;i<elements.length;i++){elements[i].hidden=false}}
var dndFieldOriginalContent=null
function showError(element,text){element.hidden=false
...
function resetDndField(dndField){var loadingField=document.getElementById(dndField.id+'--loading')
dndField.hidden=false
loadingField.hidden=true
if(dndFieldOriginalContent!==null){dndField.innerHTML=dndFieldOriginalContent}}
