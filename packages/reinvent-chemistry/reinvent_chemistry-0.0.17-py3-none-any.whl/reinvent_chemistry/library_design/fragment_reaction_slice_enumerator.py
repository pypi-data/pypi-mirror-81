...
class FragmentReactionSliceEnumerator:
            ...
            if cut == 1:
                ...
                for pair in fragment_pairs:
                    for indx, _ in enumerate(pair):
                        ...
                        # ########
                        #
                        # print(f"preparing scaffold with decorations: {self._conversions.mol_to_smiles(labeled_decoration)}")
                        ...
                        # self._validate_sliced_molecule(sliced_mol)
    ...
    def _create_sliced_molecule(self, original_sliced_mol: FragmentedMolecule, scaffold: Mol,
        ...
        # sliced_mol = FragmentedMolecule(original_sliced_mol.scaffold,original_sliced_mol.decorations)
        old_decorations = {k: v for k, v in original_sliced_mol.decorations.items()}
        ...
        # print(self._conversions.mol_to_smiles(decoration))
        # sliced_mol.add_decorations({sliced_mol.decorations_count(): decoration})

        return sliced_mol

    def _validate_sliced_molecule(self, sliced: FragmentedMolecule):
        values = [self._conversions.mol_to_smiles(smi) for num, smi in sliced.decorations.items()]
        decorations = '|'.join(values)
        molecule = self._transformation.join_scaffolds_and_decorations(sliced.scaffold, decorations)
        complete_smile = self._conversions.mol_to_smiles(molecule)
        validity = sliced.original_smiles == complete_smile

        # print(f"validity mismatch {complete_smile} {sliced.original_smiles}")
        # print(sliced.to_smiles())
        if not validity:
            print(f"validity mismatch {complete_smile} {sliced.original_smiles}")
            print(sliced.to_smiles())
        return validity
