...
from guarani.jose.jwk.jwk import JsonWebKey
from guarani.jose.exceptions import InvalidKeySetError
...
class JsonWebKeySet:
    ...
    def __init__(self, keys: list[JsonWebKey]):
        ...
        if None in ids:
            raise InvalidKeySetError("One or more keys do not have an ID.")
    ...
    def load(self, keyset: dict[str, list[dict]]) -> JsonWebKeySet:
        ...
        if not keyset or not isinstance(keyset, dict):
            raise InvalidKeySetError("Invalid JWKS format.")
