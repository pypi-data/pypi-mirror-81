...
class SQLComposer (SQL):
    ...
    def exclude (self, *Qs, **filters):
        return super ().exclude (*Qs, **{self._resolve_field (f, True): v for f, v in filters.items ()})

    def filter (self, *Qs, **filters):
        return super ().filter (*Qs, **{self._resolve_field (f, True): v for f, v in filters.items ()})
    ...
    def _resolve_field (self, k, isfilter = False):
        if not self._model:
            return k if isfilter else k.replace ('__', '.')

        try:
            fks = self._model.get_fks ()
        except NotImplementedError:
            return k

        if k in fks:
            return fks [k][0]
        fks = {'this': self._model.get_table_name ()}
        fks.update (self._fk)
        ...
        if isfilter:
            ...
            if s != -1:
                ...
                es = k.split ('__')
                if len (es) > 1 and es [-1] in OPS:
                    tail = '__' + es [-1]
                    k = '__'.join (es [:-1])

        s = k.rfind ('__')
        t = fks [k [:s]]
        return '{}.{}{}'.format (t, k [s + 2:], tail)

    def _resolve_field_haystack (self, s):
        if not self._model:
            return s
        try:
            model_fields = self._model.get_fks ()
        except NotImplementedError:
            return s
        ...
        fks = {'this': self._model.get_table_name ()}
        fks.update (self._fk)
        for fk, jtable in sorted (fks.items (), key = lambda x: len ( [0]), reverse = True):
            rx = re.compile (r"(^|[^_.a-z0-9'])(?:{}__)([a-z0-9])".format (fk), re.I)
            s = rx.sub (r'\1{}.\2'.format (jtable), s)
        for fk, (column, model) in model_fields.items ():
            rx = re.compile (r"(^|[^_.a-z0-9'])(?:{})($|[^_a-z0-9])".format (fk), re.I)
            s = rx.sub (r'\1{}\2'.format (column), s)
        return make_list and s.split (", ") or s

    def _join_select_fields (self):
        super ()._join_select_fields ()
        if "select" not in self._feed or not self._model:
            return
        self._feed ['select'] = self._resolve_field_haystack (self._feed ['select'])

    def order_by (self, *by):
        return super ().order_by (*self._resolve_field_haystack (by))

    def group_by (self, *by):
        return super ().group_by (*self._resolve_field_haystack (by))

    def _join_related (self, jtype, fk_chain):
        ...
        on = '{}.{} = {}.{}'.format (table, fk_column, target_table, target_pk)
        if jtype == "FROM":
            self._join (jtype, target_table, '', on = None)
            return self.filter (on)
        return self._join (jtype, target_table, '', on = on)

    def join_related (self, fk_chain):
        return self._join_related ("INNER JOIN", fk_chain)

    def left_join_related (self, fk_chain):
        return self._join_related ("LEFT OUTER JOIN", fk_chain)

    def right_join_related (self, fk_chain):
        return self._join_related ("RIGHT OUTER JOIN", fk_chain)

    def full_join_related (self, fk_chain):
        return self._join_related ("FULL OUTER JOIN", fk_chain)

    def cross_related (self, fk_chain):
        return self._join_related ("CROSS JOIN", fk_chain)

    def from_related (self, fk_chain):
        return self._join_related ("FROM", fk_chain)
