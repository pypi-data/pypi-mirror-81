...
class Transfer2SQLDB(object):
    ...
    def bring_data_from_table(self, table_name: str) -> pd.DataFrame:
        tmp_df = self.__cursor.execute("select * from {}".format(table_name))
        #tmp_df = pd.DataFrame(self.__cursor.fetchall())
        for key in list(x.Field for x in self.__cursor.execute("describe {}".format(table_name)).itertuples() if x.Type == "datetime"):
            print(key)
