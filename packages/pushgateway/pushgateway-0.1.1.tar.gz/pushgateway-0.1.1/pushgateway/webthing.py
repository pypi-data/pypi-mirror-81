...
class WebSocketStream:
    ...
    def __init__(self, metadata: Metadata, on_property_changed_callback):
        self.logger = logging.getLogger("webthing." + metadata.name)
        ...
        while self.is_running:
            try:
                ...
                try:
                    ...
                    self.logger.info('webthing property ' + self.metadata.name + ' websocket ' + self.metadata.prop_ws_uri + ' connected')
                    while self.is_running:
                    ...
                    self.logger.info('websocket ' + self.metadata.prop_ws_uri + ' disconnected')
                    ws.close()
            except Exception as e:
                self.logger.error("error occurred consuming web socket for " + self.metadata.name + " " + str(e))
                time.sleep(5)
...
class WebthingProperty:
    ...
    def __init__(self, webthing_uri: str, webthing_property: str):
        self.logger = logging.getLogger("webthing." + webthing_property)
    ...
    def metadata(self) -> Metadata:
        if self.__metadata is None:
            self.logger.info('webthing property ' + self.__webthing_property + ' fetching meta data')
            ...
            self.logger.info('webthing property ' + self.__webthing_property + " meta data loaded (type: " + self.__metadata.type + ", readonly: " + str(self.__metadata.readonly) + ")")
    ...
    def property(self, value):
        try:
            ...
            self.logger.error("got error by webthing property " + self.name + " = " + str(value) + " reason: " + resp.text)
