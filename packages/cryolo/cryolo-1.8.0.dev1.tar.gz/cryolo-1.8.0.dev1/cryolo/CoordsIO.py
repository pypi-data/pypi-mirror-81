...
def write_star_file(path, boxes):
    coords = np.zeros(shape=(len(boxes),2))
...
def read_star_file(path, box_width):
    ...
    for coords_index in range(len(xcoords)):
        bound_box = BoundBox(
            x=xcoords[coords_index] - box_width / 2, y=ycoords[coords_index] - box_width / 2, w=box_width, h=box_width
        )
    ...
    try:
        ...
        if 'cryolo' in starfile:
            for i in range(len(starfile['cryolo']['_CoordinateX'])):
                ...
                if starfile['cryolo']["_EstWidth"][i] == -1 and \
                        starfile['cryolo']["_EstHeight"][i] == -1:
...
def write_cbox_file(path, boxes, additional_slice_include=None, additional_slice_exclude=None):
    num_boxes = len(boxes)
    coords = np.zeros(shape=(num_boxes, 9))
    ...
    include_slices = np.unique(coords[:,2]).tolist()
