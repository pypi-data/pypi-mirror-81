...
def read_global_config() -> Dict[Text, Any]:
    ...
    try:
        return convo.utils.io.read_config_file(GLOBAL_USER_CONFIG_PATH)
    except Exception:
...
def set_log_level(log_level: Optional[int] = None):
    ...
    logging.getLogger("convo").setLevel(log_level)
...
def write_global_config_value(name: Text, value: Any) -> None:
    ...
    try:
        ...
        c[name] = value
        convo.core.utils.dump_obj_as_yaml_to_file(GLOBAL_USER_CONFIG_PATH, c)
