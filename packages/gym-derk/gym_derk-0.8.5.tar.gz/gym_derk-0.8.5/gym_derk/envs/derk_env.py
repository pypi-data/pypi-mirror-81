...
ENVS_SINGLE_LOCAL: DerkEnvList = [['ws://localhost:8788', [['both', 0, -1]]]]
ENVS_DUAL_LOCAL: DerkEnvList = [['ws://localhost:8788', [['home', 0, -1]]], ['ws://localhost:8789', [['away', 0, -1]]]]
...
class DerkEnv(gym.Env):
  ...
  * ``mode="connected"``: Connects to an environment running in "server" mode.
  ...
  Args:
    ...
    server_host: Set host in server mode. Defaults to "localhost". (Environment variable: DERK_SERVER_HOST)
  ...
  def __init__(self,
    ...
    self.server_host = server_host if server_host is not None else os.environ.get('DERK_SERVER_HOST', 'localhost')
    ...
    start_server = websockets.serve(self._handle_websocket, self.server_host, self.server_port)
    self.websocket_server = asyncio.get_event_loop().run_until_complete(start_server)
    self.remote_messages = asyncio.Queue()
    print('Serving on ws://' + self.server_host + ':' + str(self.server_port))
  ...
  async def _connect_environments(self):
    ...
    await self.page.evaluate('''(envs, nArenas) => window.derk.connectToEnvironments(envs, nArenas)''', envs, self.n_arenas)
  ...
  async def async_reset(self):
    ...
    if self.page:
      if self.safe_reset:
        ...
        await self.page.waitForSelector('.GymLoaded')
        logger.info('[reset] Gym loaded ok')
        await self._connect_environments()
      config = {
        'home': self.home_team,
        'away': self.away_team,
      ...
      asyncio.get_event_loop().create_task(self.page.evaluate('''(config) => window.derk.runEpisode(config)''', config))

    self.total_reward = np.zeros((self.n_agents))
    logger.info('[reset] Waiting for observations')
    observations = self._decode_observations(await self._get_msg())
    logger.info('[reset] Got observations')
    return observations
  ...
  async def async_close(self):
    ...
    logger.info('[async_close] Closing environment')
    self.websocket_server.close()
    if self.browser:
      await self.browser.close()
    if self.internal_http_server:
      self.bundle_server.shutdown()
    logger.info('[async_close] Done')
