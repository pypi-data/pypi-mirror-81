...
class DeployCommand(StartCommand):
    ...
    def deployproject(self):
        ...
        if response.text == "It seems that is already an architecture deployed":
            uri, created = self.create_folder_projects(project_name = name_project)
            if not created:
                print("Good job datupian, now you can develop the project and make commits for CI/CD")
            else:
                response_status = requests.post(self.uri_status,json=data)
                if os.path.exists(f"{uri}/{name_project}"):
                    with open(f"{uri}/{name_project}/{name_project}.tf.json", "w") as _file:
                        json.dump(response_status.json(),_file)
                    with open(f"{uri}/{name_project}/project.cfg", "w") as _file:
                        _file.write("[metadata]\n")
                        _file.write(f"project = {name_project}\n")
                        _file.write(f"architecture = {uri}/{name_project}/{name_project}.tf.json\n")                
