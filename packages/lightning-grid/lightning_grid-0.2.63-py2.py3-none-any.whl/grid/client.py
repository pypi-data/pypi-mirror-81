...
from grid.observables import Run, Experiment, InteractiveNode
...
class Grid(CredentialsMixin):
    ...
    def _load_global_settings(self) -> None:
        ...
        if 'debug' in user_settings:
            env.DEBUG = bool(user_settings['debug'])

        if 'ignore_warnings' in user_settings:
            env.IGNORE_WARNINGS = bool(user_settings['ignore_warnings'])

    def _init_client(self,
                     websocket: bool = False) -> None:  # pragma: no cover
        """
    ...
    @staticmethod
    def _execute_git_command(args: List[str]) -> str:
        """
    ...
    def _add_git_root_path(entrypoint: str) -> str:
        ...
        repository_path = Grid._execute_git_command(
    ...
    @staticmethod
    def _check_if_uncommited_files():
        """
        ...
        """
        files = Grid._execute_git_command(['update-index', '--refresh'])
   ...
   """
    ...
    @staticmethod
    def _check_if_remote_head_is_different():
        """
        ...
        """
        # Check SHA values.
        local_sha = Grid._execute_git_command(['rev-parse', '@'])
        remote_sha = Grid._execute_git_command(['rev-parse', r"@{u}"])
        base_sha = Grid._execute_git_command(['merge-base', '@', r"@{u}"])
    ...
    Your local git repository seems to be out-of-sync
    with GitHub. Grid will only have access to the files
    avalable on GitHub when running your experiments.
   ...
   """
    ...
    def _check_user_github_token(self) -> None:
        """
        ...
        try:
            ...
            if not result['checkUserGithubToken']['hasValidToken']:
    ...
    def _check_github_repository() -> None:
        ...
        github_repository = Grid._execute_git_command(
            ["config", "--get", "remote.origin.url"])
        env.logger.debug(github_repository)

        if not github_repository and 'github.com' not in github_repository:
            raise click.ClickException(
    ...
    def login(self, username: str, key: str) -> None:
        """
    ...
    def train(self, config: str, kind: WorkflowType, run_name: str,
        ...
        #  Get commit SHA
        commit_sha = Grid._execute_git_command(['rev-parse', 'HEAD'])
    ...
    def history(self,
                ...
                kind: Optional[ObservableType] = None) -> None:
        """
    ...
    def cancel(self,
        ...
        for experiment in experiments:
            ...
            if experiment_status not in non_cancelled_statuses:
                ...
                spinner = yaspin(text=f'Cancelling {experiment_id}',
                                 color="yellow")
                spinner.start()
                ...
                try:
                    ...
                    if result and result['cancelExperiment']['success']:
                        spinner.ok("✔")
    ...
    def create_interactive_node(self, config: str, name: str) -> None:
        """
        ...
        spinner = yaspin(text="Creating Interactive node ...", color="yellow")
        spinner.start()
        ...
        if result['createInteractiveNode']['success']:
            spinner.ok("✔")
            click.echo(f'Interactive node {name} is spinning up.')

        elif not result['createInteractiveNode']['success']:
            spinner.fail("✘")
            if env.DEBUG:
    ...
    def delete_interactive_node(self, interactive_node_id: str) -> None:
        ...
        interactive_node_id: str
            Interactive node ID
        """
        spinner = yaspin(text="Deleting Interactive node ...", color="yellow")
        spinner.start()
        ...
        if result['deleteInteractiveNode']['success']:
            spinner.ok("✔")
            click.echo(f'Interactive node {interactive_node_id} has ' +
                       'been deleted successfully.')

        elif not result['deleteInteractiveNode']['success']:
            spinner.fail("✘")
            if env.DEBUG:
    ...
    def experiment_logs(self,
        ...
        """
        #  Starts spinner.
        spinner = yaspin(text="Fetching logs ...", color="yellow")
        spinner.start()
    ...
    def download_experiment_artifacts(self, experiment_id: str,
        ...
        """
        #  Starts spinner.
        spinner = yaspin(text="Downloading artifacts ...", color="yellow")
        spinner.start()
    ...
    def delete(self,
        ...
        if experiment_id:
            ...
            try:
                result = self.client.execute(mutation, variable_values=params)
                if result and result['deleteExperiment']['success']:
                    spinner.ok("✔")
                    click.echo(
            ...
            except Exception as e:  # skipcq: PYL-4
                spinner.fail("✘")
                raise click.ClickException(
            ...
            try:
                result = self.client.execute(mutation, variable_values=params)
                if result and result['deleteRun']['success']:
                    spinner.ok("✔")
    ...
    def upload_datastore(self,
        ...
        try:
            ...
            total_size = tar_directory_unix(source_dir=source_dir,
                                            temp_dir=os.path.basename(
                                                temp_dir.name),
                                            target_file=target_file)

            spinner.text = "Uploading datastore to S3..."
            uploader = S3DatastoreUploader(user_id=self.user_id,
