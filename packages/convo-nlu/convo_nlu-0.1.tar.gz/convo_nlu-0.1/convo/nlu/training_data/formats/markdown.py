...
from convo.constants import DOCS_URL_TRAINING_DATA_NLU
from convo.core.constants import INTENT_MESSAGE_PREFIX

from convo.nlu.training_data.formats.readerwriter import (
    TrainingDataReader,
    TrainingDataWriter,
)
from convo.nlu.utils import build_entity
...
class MarkdownReader(TrainingDataReader):
    ...
    def _get_validated_dict(json_str: Text) -> Dict[Text, Text]:
        ...
        """
        import json
        import convo.utils.validation as validation_utils
        import convo.nlu.schemas.data_schema as schema
    ...
    def _add_synonym(self, text: Text, value: Text) -> None:
        from convo.nlu.training_data.util import check_duplicate_synonym
...
class MarkdownWriter(TrainingDataWriter):
    ...
    def _generate_training_examples_md(self, training_data: "TrainingData") -> Text:
        ...
        for example in [e.as_dict_nlu() for e in training_data.training_examples]:
            convo_nlu_training_data_utils.remove_untrainable_entities_from(example)
