...
from convo.constants import MINIMUM_COMPATIBLE_VERSION
from convo.nlu import components, utils  # pytype: disable=pyi-error
from convo.nlu.components import Component, ComponentBuilder  # pytype: disable=pyi-error
from convo.nlu.config import ConvoNLUModelConfig, component_config_from_pipeline
from convo.nlu.persistor import Persistor
...
class Metadata:
    ...
    def load(model_dir: Text):
        ...
        try:
            ...
            data = convo.utils.io.read_json_file(metadata_file)
...
class Trainer:
    ...
    def _build_pipeline(
        self, cfg: ConvoNLUModelConfig, component_builder: ComponentBuilder
    ) -> List[Component]:
    ...
    def persist(
        ...
        convo.utils.io.create_directory(dir_name)
...
class Interpreter:
    ...
    def ensure_model_compatibility(
        ...
        model_version = metadata.get("convo_version", "0.0.0")
        if version.parse(model_version) < version.parse(version_to_check):
            raise UnsupportedModelError(
                ...
                "".format(model_version, convo.__version__)
            )
    ...
    def load(
        ...
        Args:
            ...
            component_builder: The
                :class:`convo.nlu.components.ComponentBuilder` to use.
