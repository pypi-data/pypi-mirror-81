...
from http_server_base.tools.errors import SubrequestFailedErrorCodes, SubrequestFailedError
from http_server_base.tools.extensions import revrange
...
class SubrequestClient(RequestLoggerClient, ErrorMatcher):
    ...
    #region Fetching
    async def _fetch__form_request(self, request: Union[str, HTTPRequest], *, query: Union[str, Dict[str, Any], None], add_request_id: bool, files: Optional[Iterable[Tuple[str, str, AnyStr, str]]], encode_body: Optional[str], expected_content_type: Optional[str] = None, **kwargs) -> HttpSubrequest:
        ...
        if (encode_body):
            request = await self._fetch__form_request__encode_body(request, encode_body=encode_body, files=files)
        
        if (query):
            request = await self._fetch__form_request__encode_query(request, query=query)
    ...
    async def _fetch__form_request__add_accept_header(self, request: HttpSubrequest, *, expected_content_type: str) -> HttpSubrequest:
        ...
        return request
    async def _fetch__form_request__encode_query(self, request: HttpSubrequest, *, query: Union[str, Dict[str, Any], None]) -> HttpSubrequest:
        ...
        return request
    async def _fetch__form_request__encode_body(self, request: HttpSubrequest, *, files: Optional[Iterable[Tuple[str, str, AnyStr, str]]], encode_body: Optional[str]) -> HttpSubrequest:
        body = request.body
        ...
        if (encode_body is None):
            ...
            if (body is None):
                ...
                if (body):
                    encoded_obj = self.model_encoder.encode_many(type(body[0]), body)
                else:
                    encoded_obj = [ ]
            else:
                encoded_obj = self.model_encoder.encode_single(type(body), body)
            
            if (encode_body.startswith('multipart/')):
                if (isinstance(encode_body, dict)):
                    encode_body = ((str(k), str(v)) for k, v in encode_body.items())
                content_type_extensions, body = self.encode_multipart_form_data(fields=encode_body, files=files)
        ...
        return request
    @classmethod
    def encode_multipart_form_data(cls, fields: Optional[Iterable[Tuple[str, str]]] = None, files: Optional[Iterable[Tuple[str, str, AnyStr, str]]] = None, *, boundary: AnyStr = '--------------------------F1ELD$_$PL117ER__$$', line_separator: AnyStr = b'\r\n', encoding: str = 'utf-8') -> Tuple[str, bytes]:
        """
        :param fields:
        (name, value) tuple.
    ...
    async def _fetch__check_response(self, response: HttpSubrequestResponse, *, expected_codes: Collection[int], expected_content_type: Optional[str]):
        ...
        if (exception is None and response.code not in expected_codes):
            SubrequestFailedError(response, expected_codes=expected_codes)
    ...
    async def fetch_json(self, request: Union[str, HTTPRequest], *, check_content_type_header: bool = True, json_load_options: Dict[str, Any] = None, **kwargs) -> Tuple[HttpSubrequestResponse, JsonSerializable]:
        ...
        try:
            ...
            raise SubrequestFailedError(response, base_error=e, message=error_message, error_code=SubrequestFailedErrorCodes.InvalidResponseBody) from e
    ...
    async def fetch_binary_data(self, request: Union[str, HTTPRequest], **kwargs) -> Tuple[HttpSubrequestResponse, bytes]:
        ...
        try:
        ...
        except SubrequestFailedError as base_error:
            response = base_error.response
            resp_data = None
            ...
            try:
                ...
                raise SubrequestFailedError(response, base_error=base_error, code=503, message=f"Unexpected error code: {response.code}, response is not JSON: '{resp_data}'", server_response=str(resp_data)) from e
            except Exception as e:
                raise SubrequestFailedError(response, base_error=base_error, code=500, error="Unknown Error", message=f"Unknown error: {e}") from e
            else:
                raise SubrequestFailedError(response, base_error=base_error, message=resp_json.get('reason') or resp_json.get('error') or (resp_json.get('errors') and resp_json['errors'][0]) or resp_json.get('message'), server_response=resp_json) from base_error
        
        if (response.body is None):
            raise SubrequestFailedError(response, base_error=base_error, message="Server responded empty body", error_code=SubrequestFailedErrorCodes.InvalidResponseBody)
