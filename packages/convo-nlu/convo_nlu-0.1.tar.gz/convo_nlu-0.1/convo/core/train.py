...
async def get_no_of_stories(story_file: Text, domain: Text) -> int:
    ...
    from convo.core.domain import TemplateDomain
...
async def do_compare_training(
    ...
    convo.utils.io.dump_obj_as_json_to_file(training_stories_per_model_file, story_range)
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `convo.core.train` directly is no longer supported. Please use "
        "`convo train` to train a combined Core and NLU model or `convo train core` "
