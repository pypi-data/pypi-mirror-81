...
from convo.utils.common import raise_warning, arguments_of
...
from convo.core.domain import InvalidDomain
from convo.core.events import Event
...
from convo.core.trackers import DialogueStateTracker, EventVerbosity
from convo.core.utils import AvailableEndpoints
from convo.nlu.emulators.no_emulator import NoEmulator
from convo.nlu.test import run_evaluation
...
if typing.TYPE_CHECKING:
    ...
    from convo.core.processor import MessageProcessor
...
def _create_emulator(mode: Optional[Text]) -> NoEmulator:
    ...
    if mode is None:
        ...
        from convo.nlu.emulators.wit import WitEmulator

        return WitEmulator()
    elif mode.lower() == "luis":
        from convo.nlu.emulators.luis import LUISEmulator

        return LUISEmulator()
    elif mode.lower() == "dialogflow":
        from convo.nlu.emulators.dialogflow import DialogflowEmulator
...
def create_app(
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text):
        ...
        until_time = convo.utils.endpoints.float_arg(request, "until")
    ...
    async def train(request: Request) -> HTTPResponse:
        ...
        convo.utils.io.write_text_file(rjs["config"], config_path)

        if "nlu" in rjs:
            nlu_path = os.path.join(temp_dir, "nlu.md")
            convo.utils.io.write_text_file(rjs["nlu"], nlu_path)

        if "stories" in rjs:
            stories_path = os.path.join(temp_dir, "stories.md")
            convo.utils.io.write_text_file(rjs["stories"], stories_path)

        if "responses" in rjs:
            responses_path = os.path.join(temp_dir, "responses.md")
            convo.utils.io.write_text_file(rjs["responses"], responses_path)

        domain_path = DEFAULT_DOMAIN_PATH
        if "domain" in rjs:
            domain_path = os.path.join(temp_dir, "domain.yml")
            convo.utils.io.write_text_file(rjs["domain"], domain_path)
    ...
    async def evaluate_stories(request: Request):
        ...
        stories = convo.utils.io.create_temporary_file(request.body, mode="w+b")
        use_e2e = convo.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def evaluate_intents(request: Request):
        ...
        nlu_data = convo.utils.io.create_temporary_file(request.body, mode="w+b")
