...
def read_api_key():
 ...
 if key:
  return key
 raise eQDyP('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=eQDyx)
...
def load_cached_key():
 ...
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise eQDyP('Cached key expired')
 timestamp=eQDyp(cached_key['timestamp'])
