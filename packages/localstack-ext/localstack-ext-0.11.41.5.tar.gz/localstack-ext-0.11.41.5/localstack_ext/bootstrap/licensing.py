...
def read_api_key():
 ...
 if key:
  return key
 raise XBJLw('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=XBJLP)
...
def load_cached_key():
 ...
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise XBJLw('Cached key expired')
 timestamp=XBJLW(cached_key['timestamp'])
