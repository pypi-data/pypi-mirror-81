...
def prepare_setup(component, target):
    ...
    params = {
        ...
        'url': conf['repository']['giturl'],
        'customer': conf['org']['name']
    }
...
def test_conf_pipeline(conf, context):
    ...
    for t in conf['transforms']['enabled']:
        name = t['transform']
...
def testdata_spec_show(specfile, capture, context, name):
    ...
    print("------")
    print("[Targets] Available " + "(files)" if dataset.isfile else "(dirs)")
    print("------")
    ...
    print("------")
    print("[Download] Possible Commands")
    print("------")
    ...
    for c in registry.commands:
        ...
        if ((combination[0] not in targets) or
            (combination[1] not in targets)):
            print("  ", redbg("No valid combinations"))
            continue

        found = False
        for name1 in targets[combination[0]]:
            for name2 in targets[combination[1]]:
                if name1 == name2:
                    continue
                print("   ",greenbg("{} => {}".format(name1, name2)))
                found = True
        if not found:
            print("  ", redbg("No valid combinations"))


def testdata_transform_download(pkgdir, capture, context,
...
def testdata_spec_download(specfile, capture, context, name,
                           command, src, dst, start, end):
    """
    Show details of a dataset
    ...
    # Get more details of all
    try:
        src = dataset.get_path_by_name(src, full=True)
    except:
        print(redbg("Source {} could not be found".format(src)))
        return

    try:
        dst = dataset.get_path_by_name(dst, full=True)
    except:
        print(redbg("Dst {} could not be found".format(dst)))
        return
    ...
    if src['nature'] != command['valid'][0]:
        print(redbg("src nature found {} expected {} ".format(src['nature'],
                                                             command['valid'][0])))
        return

    if dst['nature'] != command['valid'][1]:
        print(redbg("Dst nature found {} expected {}".format(dst['nature'],
                                                             command['valid'][1])))
        return


    src['path'] = config.get_file(src['path'],
                                  extra=registry.params,
                                  abspath=False)
    dst['path'] = config.get_file(dst['path'],
                                  extra=registry.params,
                                  abspath=False)
    ...
    try:
        ...
        print(redbg("Invalid start or end date"))
        return

    print("[commands] Please run these after checking")
    combinations = dataset.generate(end=end,start=start)
    for c in combinations:
        params = copy.copy(registry.params)
        params['source'] = src['path']
        params['destination'] = dst['path']
...
def testdata_action(action, *args, **kwargs):
    ...
    if os.path.isdir(spec):
        ...
        if action == 'list':
            ...
            testdata_transform_download(*args, **kwargs)
    else:
        if action == 'list':
            ...
            testdata_spec_download(*args, **kwargs)
