...
from qary.constants import DATA_DIR, QUESTION_STOPWORDS, ES_HOST
...
def md5(s: str, num_bytes=8, dtype=np.uint64):
    """ Like builtin hash but consistent across processes and accepts custom dtype (default npuint64)
...
def normalize_title(title, lowerer=str.lower):
    """ Lowercase and remove all non-alpha characters, including whitespace

    >>> normalize_title("_Hello \t g00d-World_ !!!")
    'hellogdworld'
    """
    return re.sub(r'[^a-z]', '', str(title).lower())
...
def hashed_titles_series(titles, dtype=np.uint32):
    ...
    dtype: object
    >>> md5(normalize_title('!!HELLO!!')) in _.index
    True
    """
    ...
    titles = np.array([normalize_title(str(t)) for t in titles])
...
def save_hashes(hashes=None,
    ...
    if hashes is None:
        hashes = read_and_hash_titles(filepath=filepath)
...
def read_and_hash_titles(filepath=None, chunksize=100000, numchunks=None):
    ...
    for i, chunk in enumerate(tqdm(pd.read_csv(filepath, chunksize=chunksize))):
        ...
        hashes += list(
            md5(normalize_title(str(title))) for title in titles)
...
class WikiScraper:
    ...
    def __init__(self,
        ...
        self.es_host = ES_HOST if (es_host and (es_host is True or str(es_host).lower().strip() == 'default')) else es_host
        self.es_host = self.es_host or None
        ...
        if self.hashes_filepath:
            ...
            self.title_hashes = read_hashes(self.hashes_filepath) if self.hashes_filepath.is_file() else None
    ...
    def get_elastic_page(self, title: str):
        ...
        if self.es_host:
            ...
            page_dict = elastic_results_as_page_dict(elastic_page)
        return page_dict
    ...
    def filtered_titles(self, titles):
        ...
        for title in titles:
            ...
            if self.title_hashes is not None:
                if md5(normalize_title(title)) in self.title_hashes:
                    filtered_titles.append(title)
            elif len(normalize_title(title)) > 2:
                filtered_titles.append(title)
    ...
    def scrape_article_pages(self,
        ...
        while num_articles < max_articles and d <= max_depth and queue_pos < len(title_depths):
            title = ''
            ...
            log.info(str([d, max_depth, num_articles, title]))
            yield page_dict
    ...
    def find_article_texts(self,
        ...
        return self.scrape_article_texts(
            titles, max_depth=max_depth, max_articles=max_articles, **scrape_kwargs)
