...
class AuthProviderBase(ABC):
    ...
    subclassing :class:`.Identity` or a :class:`UserInfo` object respectively.
...
def init_auth_providers(app):
    ...
    for config in auth_providers:
        ...
        auth_classes = auth_provider_types[provider_type]

        provider_class = get_class_by_name(auth_classes["provider"])
        config["provider_class"] = provider_class

        identity_class = get_class_by_name(auth_classes["identity"])
        config["identity_class"] = identity_class

        form_class = get_class_by_name(auth_classes["form"])
