        ...
        if self.verify_token:
            ...
            async def check_token(
                ...
                if mode == "subscribe" and verify_token == self.verify_token:
                    ...
                    return PlainTextResponse("cannot validate token", status_code=403)
            ...
            async def pass_thru(
                ...
                """
                logger.warning(
                    "Without verify token supplied, It is possible for messing up about facebook development app callback."
                )
                return challenge
        ...
        async def handle_entry(
            ...
            if self.app_secret:
                if self.verify_webhook(the_signature, self.app_secret, body):
                    logger.debug("verify webhook with app secret success")
                else:
                    logger.error("fail to verify webhook with given app secret")
            else:
                logger.warning(
                    "Without app secret supplied, The server will not be able to identity the integrity of callback."
                )
