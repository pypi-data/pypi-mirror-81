...
class Export(WMLResource):
    ...
    def start(self, meta_props, space_id=None, project_id=None):
        """
            ...
            Either ALL_ASSETS or ASSET_TYPES or ASSET_IDS has to be given. Only on of these can be provided

            Only for models, dependent assets are also exported automatically. For any otehr asset type, provide
            the dependent asset ids explicitly under ASSET_IDS list. In this case, ALL_ASSETS or ASSET_TYPES can't be
            used since they won't export dependent assets
           ...
           **Example**
                ...
                >>>    client.export_assets.ConfigurationMetaNames.ALL_ASSETS: False,
        ...
        details = self._handle_response(expected_status_code=200,
                                        operationName=u'export start',
                                        response=creation_response)

        export_id = details[u'metadata']['id']

        print("export job {} has started. Monitor status using client.export_assets.get_details({})".format(export_id,export_id))
    ...
    def get_exported_content(self, export_id, space_id=None, project_id=None, file_path=None):
        """
            Get the exported content as a tar file
            ...
            **Example**
             ...
             >>>                                     space_id= '98a53931-a8c0-4c2f-8319-c793155e4598'
             >>>                                     file_path='/home/user/my_exported_content.tar.gz')
        """
        if not self._client.CLOUD_PLATFORM_SPACES and not self._client.ICP_PLATFORM_SPACES:
            raise WMLClientError("Not supported in this release")
