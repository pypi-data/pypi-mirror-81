...
class DBReader:
	...
	def read_ts_day_data(
		...
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'L1_tushare_data.sqlite3'),
			'ts_day_data'
		)
	...
	def read_ts_moneyflow_hsgt(
		...
		str_sql = \
			...
			+ str_start + ' AND ' + dbname_date + '<=' + str_end + ')'
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L1_tushare_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
			raise ConnectionError('数据库数据获取失败，请确保tushare_data库存在')
	...
	def read_tdx_1min_data(
		...
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'L1_tdx_1min_data.sqlite3'),
			'tdx_1min_data'
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		...
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L1_tdx_1min_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
			raise ConnectionError('数据库数据获取失败，请确保tushare_data库存在')
	...
	def read_jj_1min_index_data(
		...
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'L1_jj_data.sqlite3'),
			'jj_1min_index_data'
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		...
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L1_jj_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def read_fundamentals(
		...
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'L1_fundamentals.sqlite3'),
			'fundamentals'
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		...
		str_sql = \
			...
			+ dbname_date + '>=' + str_start + ' AND ' + dbname_date + '<=' + str_end + ')' + str_stardrop
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L1_fundamentals.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def read_strategy(
		...
		str_sql = \
			...
			str_stardrop
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L3_strategy_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def read_mk_tech_day_data(
		...
		dbname_code = 'code'
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'mk_data.sqlite3'),
			'mk_tech_day_data'
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		...
		str_sql = \
			...
			+ dbname_date + '>=' + str_start + ' AND ' + dbname_date + '<=' + str_end + ')' + str_stardrop
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'mk_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def read_cal_market_1min_data(
		...
		dbname_date = 'datetime'
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], 'L2_cal_market_data.sqlite3'),
			'cal_market_1min_data'
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		str_start, str_end = \
		...
		try:
			connect = \
				sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], 'L2_cal_market_data.sqlite3'))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def query_stock_day(
		...
		str_sql = \
			...
			+ dbname_date + '>=' + str_start + ' AND ' + dbname_date + '<=' + str_end + ')' + str_stardrop
		try:
			connect = sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], dbname))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
	...
	def query_stock_min(
		...
		dbname = self.__check_dbtablename(dbname, tablename)
		col = self.get_tableinfo_cols(
			os.path.join(self.config.loc[0, 'dbpath'], dbname), tablename)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
		str_code = self.__check_code(code=code, dbname_code=dbname_code)
		str_start, str_end = \
	...
	def query_market_min(
		...
		try:
			connect = \
				sqlite3.connect(os.path.join(self.config.loc[0, 'dbpath'], dbname))
			df = pd.read_sql(str_sql, con=connect)
			connect.close()
		except Exception:
			raise ConnectionError(f'数据库数据获取失败，请确保{dbname}库以及{tablename}表存在')
	...
	def query_stock_limitday(
		...
		dbname_code = 'code'
		dbname = self.__check_dbtablename(dbname, tablename)
		col = self.get_tableinfo_cols(os.path.join(self.config.loc[0, 'dbpath'], dbname), tablename)
