define([
    ...
    , 'nbextensions/visualpython/src/component/fileNavigation/executor'
              ...
              componentConstApi, fileNavigationExecutorFuncList) {
    
    // kernel executor에 필요한 함수 import
    var { executeKernelFromDir
          , executeKernelFromDirLeftTab
          , executeKernelFromDirRightBody } = fileNavigationExecutorFuncList;
    // dom 만드는데 필요한 함수 import
    var { makeDom, controlToggleInput, closeComponent } = componentConstApi;
    ...
    FileNavigationRenderer.prototype.initRender = function() {
        ...
        executeKernelFromDirRightBody(dirObj, fileNavigationRendererThis);
        ...
        if(fileNavigationtype === 'SAVE_FILE'){
            ...
            // $(`.directoryComponent-menu`).append(saveFileDataBtn);
            // $(`.directoryComponent-menu`).append(saveFileDataInput);
            controlToggleInput();
    ...
    /** */
    FileNavigationRenderer.prototype.renderCurrentDirPathInfoLeftTab = function(dirInfoArr, rootDom) {
        var { folderArr } = this.makeFolderOrFileDomArr(dirInfoArr , 'tab');
        folderArr.forEach(folder => {
            rootDom.appendChild(folder);
        });
    }
    ...
    FileNavigationRenderer.prototype.finishLoadingBarAndSetCurrDirStr = function(currentDirStr, currentRelativePathStr) {
        var fileNavigationRendererThis = this;
        fileNavigationRendererThis.fileNavigationState.setRelativePathStr(currentRelativePathStr);
        fileNavigationRendererThis.fileNavigationState.setCurrentDirStr(currentDirStr);

        $('.directoryComponent-container').find('#vp_loadingBar').remove();
        // $('.directoryComponent-directory-nowLocation').html(`/${currentRelativePathStr}`);
    }
    ...
    FileNavigationRenderer.prototype.renderNowLocation = function(currentDirStr, currentRelativePathStr) {
        ...
        var fileNavigationRendererThis = this;
        var fileNavigationState = this.getFileNavigationState();
        fileNavigationState.setRelativePathStr(currentRelativePathStr);
        fileNavigationState.setCurrentDirStr(currentDirStr);
        var notebookPathStr = fileNavigationState.getBaseFolderStr();
        var notebookPathStr = fileNavigationState.getNotebookPathStr();
    ...
    FileNavigationRenderer.prototype.selectFile = function(relativeDirPath, filePathStr) {
        ...
        var baseFolder = fileNavigationState.getBaseFolderStr();
        var baseDirStr = fileNavigationState.getBaseDirStr();
        var noteBookPathStr = fileNavigationState.getNotebookPathStr();
        var currentDirStr = fileNavigationState.getCurrentDirStr();
        var noteBookPathStrLength = noteBookPathStr.length;
        ...
        if (fileNavigationState.getFileNavigationtype() === 'SAVE_FILE') {
            $(document).trigger("fileSaveSelected.fileNavigation");
        } else {
            $(document).trigger("fileReadSelected.fileNavigation");
        }
