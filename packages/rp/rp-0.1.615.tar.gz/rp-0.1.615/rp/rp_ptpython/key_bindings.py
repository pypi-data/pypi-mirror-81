...
def do_web_paste(buffer,repr_mode=False):
    ...
    try:
        if repr_mode:
            buffer.insert_text(repr(rp.web_paste()))
        else:
                    ...
                    if True:#not '"' in after_line and not "'" in after_line:
                        ...
                        for command,header in header_commands.items():
                            ...
                            if before_line.endswith(chopped_command) and char==key:
                                ...
                                if header=='tmux_string_paste':
                                    do_tmux_paste(buffer,repr_mode=True)
                                if header=='web_string_paste':
                                    do_web_paste(buffer,repr_mode=True)
                                if header=='tmux_paste':
                                    do_tmux_paste(buffer,repr_mode=False)
                                if header=='web_paste':
                                    do_web_paste(buffer,repr_mode=False)
                                if header=='paste':
                                    do_paste(buffer)
