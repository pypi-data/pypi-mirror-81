...
def craftandsend(targets, pid, pps, minttl=1, maxttl=32, proto=1, timer='nano'):
    ...
    cloudtrace.trace.probe.craft(targets, pid, file=tcpreplay.stdin, minttl=minttl, maxttl=maxttl, proto=proto)
    tcpreplay.stdin = None
    out, _ = tcpreplay.communicate()
    out = out.decode()
    print(out.strip())

def trace(targets: List[str], outfile: str, pps, proto, pid, waittime=30, timer='nano', read=False):
    iface, src, nexthop = conf.route.route(targets[0])
    comp = None
    ...
    proc = Process(target=craftandsend, args=(targets, pid, pps), kwargs={'proto': proto, 'timer': timer})
    proc.start()
    proc.join()
...
def main():
    ...
    print('fasttrace version: {}'.format('0.2.0'))
    ...
    while args.cycles == 0 or cycle < args.cycles:
        ...
        trace(targets, filename, args.pps, args.proto, pid, waittime=args.wait, timer=args.timer, read=args.read)
        if args.remote:
