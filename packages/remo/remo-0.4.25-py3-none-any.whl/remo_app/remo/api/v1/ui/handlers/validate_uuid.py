...
class ValidateUUID(viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {'valid': AgrInstallations.objects.filter(uuid=uuid).exists()}, status=status.HTTP_200_OK
        )
    ...
    def trial(self, request, pk=None):
        ...
        if not AgrInstallations.objects.filter(uuid=uuid).exists():
        ...
        return Response({'valid': total_usage < timedelta(hours=10)}, status=status.HTTP_200_OK)
