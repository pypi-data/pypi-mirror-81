...
def smart_loader(loader: Callable = None, *, support_ref_field: bool = True):
...
class OpenApiParser:
    ...
    def load_path_items(self):
        try:
            ...
            for p in paths:
                self.load_path_item(f'#/paths/[{p}]')
    # endregion
    # region Loader
    @classmethod
    def split_path_left(cls, path: str) -> Tuple[str, str, str]:
    ...
    @classmethod
    def split_path_right(cls, path: str) -> Tuple[str, str, str]:
    ...
    def load_data(self, path: str, source: Dict[str, Any] = None, *, base_path: str = None, **kwargs) -> Any:
        ...
        left, sep, right = self.split_path_left(path)
        if (left == '#'):
            if (not sep):
                return self.data
            else:
    ...
    def load_class(self, class_data: Dict[str, Any], path: str, *, is_top_level: bool, force_name: Optional[str] = None, **kwargs) -> ModelClass:
        if (force_name is not None and not is_top_level):
            ...
            _, _, name = self.split_path_right(path)
    ...
    def load_path_item(self, path_item_data: Dict[str, Any], path: str, is_top_level: bool, **kwargs) -> ModelPath:
        _, _, endpoint_path = self.split_path_right(path)
    ...
    def load_parameter(self, parameter_data: Dict[str, Any], path: str, is_top_level: bool, **kwargs) -> ModelParameter:
        ...
        if (parameter.schema is not None and not isinstance(parameter.schema, ModelSchema)):
            parameter.schema = self.load_schema(parameter.schema, f'{path}/parameters/{parameter.name}/schema', force_name=parameter.name)
    ...
    def load_request_body_object(self, request_body_object_data: Dict[str, Any], path: str, is_top_level: bool, operation_id: str = None, **kwargs) -> ModelRequestBodyObjectImpl:
        ...
        if (operation_id is None):
            operation_id = name
        request_body.content = self._load_content(request_body.content, path + '/content', force_name=operation_id + self.id_separator + 'request')
    ...
    def load_response_object(self, response_object_data: Dict[str, Any], path: str, is_top_level: bool, operation_id: Optional[str] = None, **kwargs) -> ModelResponseObject:
        ...
        if (operation_id is None):
            operation_id = name
        response.content = self._load_content(response.content, path + '/content', force_name=operation_id + self.id_separator + 'response')
        
        response.pretty_path = None
        return response
    
    def _load_content(self, content: Optional[Dict[str, Union[ModelMediaTypeObject, Dict[str, Any]]]], path: str, force_name: str) -> Optional[Dict[str, ModelMediaTypeObject]]:
        if (content is None):
            return None
