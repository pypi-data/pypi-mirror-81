...
class FlowMeter(Module,ReprDict):
    ...
    def conf(self,config=None):
        ...
        self.timeout = Timeout( timeout=self.config.flow.time_out,
                                timebase=self.config.flow.time_base )
    ...
    def load(self):
        ...
        try:
            ...
            self.config.load( "/modapp/"+mod_settings.app+"/etc/flowmeter.template.json.txt" )
        return self
