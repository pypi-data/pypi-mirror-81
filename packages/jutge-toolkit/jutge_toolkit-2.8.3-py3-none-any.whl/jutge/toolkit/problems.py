...
def verify_program(program, correct_extension='', iterations=1):
    ...
    for solution, compiler in sorted(available_list, key=lambda tup: tup[0].lower()):
        ...
        for test in tests:
            ...
            r = checkers.standard(tst + ext + ".out", tst + ".cor", pe=True)
            if r == 'AC':
                util.del_file(tst + ext + ".out")
            if r == 'PE':
                pe = True
            print((Fore.GREEN if r == 'AC' else Fore.YELLOW if r == 'PE' else Fore.RED) +
                  "%s.inp:\t\t%s\ntime: %f\t\tsize: %s" %
                  (tst, r, time, util.convert_bytes(outsize)) + Style.RESET_ALL)
        ...
        print()
        handler = util.read_yml("handler.yml")
        if pe and ('checker' not in handler or handler['checker'] != 'loosy'):
            print(Fore.YELLOW + 'Warning: Some outputs were slightly different from the correct files. Please consider using the loosy checker.\n')
