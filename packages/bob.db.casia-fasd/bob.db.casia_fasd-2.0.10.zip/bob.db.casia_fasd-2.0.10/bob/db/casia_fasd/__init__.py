...
class Database(object):
    ...
    def check_validity(self, l, obj, valid, default):
        ...
        elif isinstance(l, six.string_types) or isinstance(
                l, six.integer_types):
    ...
    """
        ...
        groups = self.check_validity(groups, "group", VALID_GROUPS,
                                     VALID_GROUPS)
        ...
        if cls == None and types != None:  # types are strictly specified which means we don't need the calss of real accesses
            ...
            cls = self.check_validity(cls, "class", VALID_CLASSES,
                                      ('real', 'attack'))

        # check if video quality types are valid
        VALID_QUALITIES = self.qualities
        qualities = self.check_validity(qualities, "quality", VALID_QUALITIES,
                                        VALID_QUALITIES)
        ...
        if cls != (
                'real',
        ):  # if the class is 'real' only, then there is no need for types to be reset to the default (real accesses have no types)
            types = self.check_validity(types, "type", VALID_TYPES,
                                        VALID_TYPES)
        ...
        for i in ids:
            for g in groups:
                if (g == 'train' and i > 20) or (g == 'test' and i <= 20):
                    continue
                cur_id = i
                if g == 'test':
                    cur_id = i - 20
                ...
                for q in qualities:
                    if cls == ('real', ) and types != None:
                        continue
                        # category real + any type does not exist
                    for c in cls:
                        if c == 'real':  # the class real doesn't have any different types, only the attacks can be of different type
                            name = os.path.join(folder_name, "%d" % cur_id,
                                                db_mappings[c + '_' + q])
    ...
    """
        if outfilename == None:
            outfilename = self.get_file(
                os.path.join(self.foldsdir, 'cross_valid.txt'))
        ...
        valid_pos = cross_valid(
            numpos, numfolds)  # the validation indices of the positive set
        valid_neg = cross_valid(
            numneg, numfolds)  # the validation indices of the negative set
  ...
  """
        ...
        for line in lines:
            ...
            if linenum % 2 == 1:
                subsets_pos.append(
                    ind_list
                )  # odd lines: validation indices for the positive class
            else:
                subsets_neg.append(
                    ind_list
                )  # even lines: validation indices for the negative class
            linenum += 1
        return subsets_pos, subsets_neg

    def cross_valid_foldfiles(self,
                              cls,
                              types=None,
                              fold_no=0,
                              directory=None,
  ...
  """
        ...
        import warnings
        warnings.warn(
            "The method Database.cross_valid_foldfiles() is deprecated, use Database.cross_valid_foldobjects() for more powerful object retrieval",
            DeprecationWarning)
        ...
        if cls == 'real':
            ...
            if 'warped' in types and 'cut' in types and 'video' in types:
                infilename = self.get_file(
                    os.path.join(self.foldsdir, 'cut_warped_video_attack.txt'))
            elif 'warped' in types and 'cut' in types:
                infilename = self.get_file(
                    os.path.join(self.foldsdir, 'cut_warped_attack.txt'))
            else:
                infilename = self.get_file(
                    os.path.join(self.foldsdir, types[0] + '_attack.txt'))

        lines = open(infilename, 'r').readlines()
        files_val = {
        }  # the keys in the both dictionaries are just pro-forma, for compatibility with other databases
        files_train = {}
        k_val = 0
        k_train = 0  # simple counters
        ...
        for line in lines:
            ...
            if int(words[1]) == fold_no:
                files_val[k_val] = make_path(words[0], directory, extension)
                k_val += 1
            else:
                files_train[k_train] = make_path(words[0], directory,
                                                 extension)
                k_train += 1

        return files_val, files_train

    def cross_valid_foldobjects(self,
                                cls,
                                types=None,
                                qualities=None,
                                fold_no=0):
        ...
        for line in lines:
            ...
            if int(words[1]) == fold_no:
                ...
                if f.get_quality() in qualities:
                    obj_val.append(
                        f
                    )  # the file still belongs to the training set, but is in dev set in cross-validation
            else:
