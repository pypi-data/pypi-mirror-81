...
def deduplication(df,cols,threshold,scorer):
        df['Matches']=''
        final_list=[]
        count=0
        df['Combined']=''
        ...
        for  i in df['Combined']:
                ...
                for found, score, matchrow in process.extract(i, df['Combined'], scorer):
                        found.rstrip('+')
                        if score >=threshold:
...
df=pd.read_csv('/Users/gandharv/Desktop/OfficeWork/DistributionPackages/dedupe-FuzzyWuzzy/dedupe_FuzzyWuzzy/messy.csv')
