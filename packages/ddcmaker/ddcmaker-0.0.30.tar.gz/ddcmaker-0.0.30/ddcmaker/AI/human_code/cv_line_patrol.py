# --*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*-+
# -*- coding: utf-8 -*-                            |
# @Time    : 2019/12/11 17:14                      *
# @Author  : Bob He                                |
# @FileName: cv_line_patrol.py                     *
# @Software: PyCharm                               |
# @Project : human_code                            *
...
def Tracing(orgimage, r_w, r_h, r=roi, l_c=line_color, l_t=line_thickness):
    ...
    orgframe = cv2.cvtColor(orgframe, cv2.COLOR_BGR2GRAY)
    orgframe = cv2.GaussianBlur(orgframe, (3, 3), 0)
    _, Imask = cv2.threshold(orgframe, 50, 255, cv2.THRESH_BINARY_INV)
    ...
    for r in roi:
        ...
        cnts, _ = cv2.findContours(blobs, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1)
        cnt_large = getAreaMaxContour(cnts)
        if cnt_large is not None:
            rect = cv2.minAreaRect(cnt_large)
            box = np.int0(cv2.boxPoints(rect))
            box[0, 1], box[1, 1], box[2, 1], box[3, 1] = box[0, 1] + (n - 1) * r_w / 4, box[1, 1] + (n - 1) * r_w / 4, \
                                                         box[2, 1] + (n - 1) * r_w / 4, box[3, 1] + (n - 1) * r_w / 4
            ...
            cv2.drawContours(frame, [box], -1, (0, 0, 255, 255), 2)
