...
'''
def wait_for_idle_cpus(required, *, fixed_delay=0.2, inertia=0.7):
    ...
    with open('/sys/fs/cgroup/cpu/cpu.cfs_quota_us', 'rt') as quota, open('/sys/fs/cgroup/cpu/cpu.cfs_period_us', 'rt') as period:
    ...
    def wait_for_slot(halt):
        ...
        while not halt():
            ...
            if idle.update() >= required and (cpu_quota is None or cpu_used.update() <= cpu_quota):
