...
def main(args=None):
    ...
    write_direct = True
    if filament_mode or predict_3D:
        write_direct = False
    try:
    ...
    if write_direct == False:
        ...
        if predict_3D:
            ...
            for result in picking_results:
                ...
                bounding_boxes_3d = grouping3d.do_tracing(boxes_list=result["boxes"],
            ...
            print(num_tomo_particles, "Particles were found by tracing",num_particles,"through the volumes.")
    ...
    if filament_mode:
        ...
        if picking_result_with_boxes:
            ...
            for picking_result_subset in picking_result_with_boxes_subsets:
                ...
                boxes_subset = [
                    picking_result_subset[i]["boxes"][0]
                    for i in range(0, len(picking_result_subset))
                ]
                print(
                    " Enhance subset ",
...
def prediction_result_to_disk(outdir, picking_results):
    ...
    for box_to_write in picking_results:
        ...
        if "is_3d" in box_to_write and box_to_write["is_3d"] == True:
            ...
            for boxes_index, boxes in enumerate(box_set):
                ...
                for boxes_index, boxes in enumerate(box_set):
                    ...
                    if outdir is not None:
                        ...
                        cbox_path = os.path.join(outdir, "CBOX_3D", filename)
            ...
            box_set = box_to_write["boxes_unfiltered"]
            #Write CBOX slices coordinates
            for boxes_index, boxes in enumerate(box_set):
                ...
                if outdir is not None:
                    filename = os.path.basename(cbox_path)
                    cbox_path = os.path.join(outdir, "CBOX_SLICE_COORDS", filename)

                # Create directory if it does not existes
                if not os.path.exists(os.path.dirname(cbox_path)):
                    os.makedirs(os.path.dirname(cbox_path))
                CoordsIO.write_cbox_file(path=cbox_path, boxes=boxes)
            ...
            for boxes_index, boxes in enumerate(box_set):
                ...
                if not os.path.exists(os.path.dirname(star_path)):
                    os.makedirs(os.path.dirname(star_path))
                CoordsIO.write_cbox_file(path=star_path, boxes=box_to_write["boxes_unfiltered"][boxes_index])
...
def setup_prediction_monitor(input_path, img_paths, monitor):
    """
...
def generate_batches(img_pths,
    ...
    for current_index_image, img_pth in enumerate(img_pths):
        ...
        if image is not None or image.shape[0] == 0 or image.shape[1] == 0:
            ...
            # Add extra loop around images here?? Do I need to adjust img_tiles? I think this
            # is not working, because there will be too many images...
...
def do_prediction(
    ...
    **kwargs
    ...
    if monitor:
        my_observer = setup_prediction_monitor(input_path, img_paths, monitor)
    ...
    if do_picking == False:
        ...
        for batch, tile_coord, slices_indicies, img_size, batch_img_paths in batch_generator:
            ...
            for batch_index, boxes in enumerate(boxes_per_image_nms_batch):
                ...
                if key in boxes_per_image_dict:
                    boxes_per_image_dict[key].append(boxes)
                    boxes_per_image_unfiltered_dict[key].append(boxes_per_image_unfiltered_batch[batch_index])
                else:
                    boxes_per_image_dict[key] = [boxes]
                    boxes_per_image_unfiltered_dict[key] = [boxes_per_image_unfiltered_batch[batch_index]]
            ...
            if len(images_completely_picked) > 0:
                ...
                for preprocessed_img_pth in images_completely_picked:
                    ...
                    boxes_filtered = boxes_per_image_dict[preprocessed_img_pth]
                    boxes_unfiltered = boxes_per_image_unfiltered_dict[preprocessed_img_pth]
            ...
            if "write_direct" in kwargs and len(images_completely_picked) > 0:
                if kwargs["write_direct"] == True:
                    ...
                    prediction_result_to_disk(
                        kwargs["outdir"], new_boxes_to_write
                    )
        ...
        # Now, in case an image contains multiple boxset, do tracing
        for box_key in boxes_per_image_dict:
            
            if len(boxes_per_image_dict[box_key])>1:
                pass
