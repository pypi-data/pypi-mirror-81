define([
    ...
    MatplotPackage.prototype.bindKindSelector = function() {
        ...
        $(selector).change(function() {
            var kind = $(this).val();
            var package = that.package[kind];
            if (package == undefined) package = that.package['plot'];
        ...
        // 플롯 차트 옵션으로 초기화
        var plotPackage = this.package['plot'];
        // 모두 숨기기 (단, 대상 변수 입력란과 차트 유형 선택지 제외)
    ...
    MatplotPackage.prototype.generateCode = function(exec) {
        ...
        try {
            ...
            if (disableChart == false) {
                ...
                var package = this.package[kind];
                // 원하는 패키지가 없으면 기본 플롯 옵션 불러오기
                if (package == undefined) {
                    package = this.package['plot'];
                }
