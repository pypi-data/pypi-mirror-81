...
class Count:
    ...
    @classmethod
    async def aouterr(cls, msg: Text = str()):
        await executor(cls.outerr, msg)

    @classmethod
    async def aoutno(cls, msg: Text = str()):
        await executor(cls.outno, msg)

    @classmethod
    async def aoutok(cls, msg: Text = str()):
        await executor(cls.outok, msg)

    @classmethod
    def console(cls, msg, error: Bool = False, no: Bool = False):
        if cls.verbose:
            if error:
                cprint(f"[bold blue]{cls.percentage}[white] "
                       f"\[ok: [bold green]{cls.ok}[white], "
                       f"error: [bold red]{cls.error}[white], "
                       f"no: [bold magenta]{cls.no}[white], "
                       f"total: [bold blue]{cls.total}[white]] "
                       f"[bold red]{msg}")
                cls.log.error(cls.format(msg))
                return
            if no:
                cprint(f"[bold blue]{cls.percentage}[white] "
                       f"\[ok: [bold green]{cls.ok}[white], "
    ...
    def format(cls, msg):
        try:
            ...
            cls.log.debug('fm IndexError')

    @classmethod
    def get_percentage(cls):
        return f'{round(cls.count * 100 / cls.total, 2)}%'

    @classmethod
    def outerr(cls, msg: Text = str()):
        cls.error += 1
        cls.count += 1
        cls.percentage = cls.get_percentage()
        cls.console(msg, error=True)

    @classmethod
    def outno(cls, msg: Text = str()):
        cls.no += 1
        cls.count += 1
        cls.percentage = cls.get_percentage()
        cls.console(msg, no=True)

    @classmethod
    def outok(cls, msg: Text = str()):
        cls.ok += 1
        cls.count += 1
        cls.percentage = cls.get_percentage()
        cls.console(msg)
...
class Env(Asdict):
    ...
    def aiodebug(self, enable: Bool = False):
        ...
        if self.file.bool(upper_prefix('DEBUG_ASYNC', prefix=self.prefix), None) or enable:
            return ok()
        return no()
...
class Ip:
    ...
    ip: OptionalText = dataclasses.field(default=urllib.request.urlopen('https://ident.me').read().decode('utf8'), init=False)
    ...
    def __post_init__(self):
        try:
            ...
            path.log.error(f'https://ident.me: {exception}')
