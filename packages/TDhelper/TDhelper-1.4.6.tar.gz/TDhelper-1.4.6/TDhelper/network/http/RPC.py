...
class RPCRegister:
    ...
    def RegisterRPC(self, serviceClass):
        self._registerService(serviceClass)
    
    def RegisterMethod(self, pk ,serviceClass):
        self._registerMehotd(pk, serviceClass)

    def _registerService(self, serviceClass):
        m_service_post_data = ''
        m_count = 0
        # 生成注册服务参数.
        ...
        if state == 200:
            ...
            if ret["state"] == 200:
                logging.info("%s(%s) register success."%(self._serviceConfig['key'],self._serviceConfig['name']))
                if ret["msg"]["id"]:
        ...
        for i in range(0, len(hosts)):
            ...
            if state != 200:
                ...
                if ret['state'] != 200:
                    ...
                    logging.info("%s register service hosts(%s:%s) success." % (
                        ...
                        try:
                            ...
                            for i in range(0, len(methods)):
                                ...
                                if not 'key' in methods[i]:
                                    methods[i]['key']= self._serviceConfig['key'].upper()+"_"+serviceClass.__name__.upper()+"_" +func_name
                                else:
                                ...
                                if state != 200:
                                    logging.error('register service methods failed. msg:{}' % str(ret, encoding='utf-8'))
                                else:
                                    ret = json.loads(str(ret, encoding='utf-8'))
                                    if ret['state'] != 200:
                                        ...
                                        logging.info("%s register success."%methods[i]['key'])
                                        if ret:
                                            if 'params' in methods[i]:
                            ...
                            logging.error(e)
                            logging.error("register params error. method config error.please checked %s.__doc__"%(v.__name__))
