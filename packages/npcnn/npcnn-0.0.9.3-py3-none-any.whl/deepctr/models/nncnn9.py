...
def NNCNN9(linear_feature_columns, dnn_feature_columns, 
         conv_kernel_width=(5, 5, 3), conv_filters=(64, 64, 64),
         mlp_feat_filters=((22, 22), (22, 22), (22, 22)), feat_pooling_width=(3, 3), feat_pooling_stride=2, dnn_hidden_units=(128, 64),
    ...
    for i in range(1, l + 1):
        ...
        if i < l:
           ...
           p_stride = feat_pooling_stride
           x = tf.keras.layers.MaxPooling2D(pool_size=(1, p_width), strides=(p_stride, p_stride))(x)
           x = tf.keras.layers.Dropout(rate = pooling_dropout_dropout)(x)
        
    x = tf.keras.layers.Flatten()(x)
    x = DNN(dnn_hidden_units, dropout_rate=dnn_dropout,l2_reg=l2_reg_dnn)(x)
