...
def plot_all_featurelist_curves(df, ts_settings, data_subset='allBacktests', metric='RMSE'):
    ...
    if data_subset == 'allBacktests':
        ...
        df[metric_column] = df[metric_column].apply(lambda x: np.mean([v for v in x if v != None]))
        df = df[['Project_Name', 'Featurelist_length', metric_column]].drop_duplicates().sort_values(
            ['Featurelist_length', 'Project_Name'], ascending=[False, True])  # [~df.isnull().any(axis=1)]
    else:
...
#     fig.update_layout(yaxis=dict(range=[min(df[metric_column].values())* 0.9,max(df[metric_column].values())* 1.1]))
    fig.show()
