...
def basic_metadata_tweets_extracted (data_tweets, node_list): 
    ...
    df_basic_metadata_tweets_extracted = pd.concat([data_tweets.drop(['user'], axis=1), data_tweets['user'].apply(pd.Series)],
                            axis=1).drop_duplicates().reset_index(drop=True)[["screen_name","followees","followers","url"]]
...
def graph_json_values(df_graph, df_basic_data):
    ...
    for i in range(len(df_basic_data)):
      ...
      data['nodes'].append({
          ...
          'followers': str(df_basic_data["followers"].iloc[i]),
          'following': str(df_basic_data["followees"].iloc[i]),
