...
class PackageInitWriter(HeaderWriter, FooterWriter, DescriptionWriter, Writer):
    ...
    def dump_package_metadata(self, parser: OpenApiParser) -> Iterator[str]:
        info: Option[OpenApiInfo] = Option(parser.metadata).map(lambda m: m.info)
        ...
        yield f"__license__ = {info.flat_map(lambda i: Option(i.licence)).map(lambda l: l.name).get_or_else('Unlicensed')!r}"
        yield f"__copyright__ = 'Copyright {datetime.now().year} {parser.author}'"
        yield f"__version__ = {info.map(lambda i: i.version).get_or_else('1.0.0')!r}"
        yield
