...
@click.command(name='cve-report', short_help="Get a CVE report")
@click.option("--runtime", help="Generate CVE report for runtime images only", type=bool)
@click.option("--scope", help="Prefix to filter the images in the report. i.e. 'gcr.io/sysdig/sysdig-agent:latest' or 'docker.io/'")
@click.option("--date", help="Specify a date for the report. Only images created after this date will be included in the report. Use format YYYY-MM-DD or \"YYYY-MM-DD HH:MM\"")
@click.option("--columns", help="List of comma-separated colums. Available columns: %s" % ", ".join(COLUMN_MAP), default="image,imageDigest,type,vuln,whitelisted,package_path,fix,cvss_v3_base_score,cvss_v2_base_score,severity,package,url", show_default=True)
...
def cve_report(cnf, runtime, scope, date, columns, report_path):
    ...
    for idx, image in enumerate(filtered_images):
        if not cnf.json:
            print("(%d/%d) Recovering vulnerability report for: %s (%s) " % (idx+1, len(filtered_images), image["fulltag"], image["imageDigest"]))
        ok, image_cves = add_cves_for(cnf, image, cve_whitelist, column_list)
        if not ok:
...
def get_whitelisted_cves(cnf):
    ok, res = cnf.sdscanning.list_whitelisted_cves()
    if not ok:
        return print_error(cnf, res)
    return [item["trigger_id"].rstrip("+*") for item in res["items"]]
