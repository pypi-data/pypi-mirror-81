...
class Env(Asdict):
    ...
    def __init__(self, prefix: Text = str()):
        ...
        self.log_level = self.file.log_level(upper_prefix('LOG_LEVEL', prefix=prefix), Logr.Levels.ERROR.value)
        self.debug_async = self.file.bool(upper_prefix('DEBUG_ASYNC', prefix=prefix), False)
        self.verbose = self.file.bool(upper_prefix('VERBOSE', prefix=prefix), False)
        self.debug = self.file.bool(upper_prefix('DEBUG', prefix=prefix), False)
...
class Ip:
    ...
    ip: OptionalText = dataclasses.field(
        default=urllib.request.urlopen('https://ident.me').read().decode('utf8'), init=False)
