...
class AFM:
    ...
    def delete_curve(self,*args):
        ...
        for curve_number in curves:
            ...
            self.approach = np.delete(self.approach, int(curve_number), axis = 1)
            self.retract = np.delete(self.retract, int(curve_number), axis = 1)
            self.z_position = np.delete(self.z_position, int(curve_number), axis = 1)

            try:
                self.adhesion = np.delete(self.adhesion, int(curve_number), axis = 1)
            except:
                pass

            try:
                self.modulus = np.delete(self.modulus, int(curve_number), axis = 1)
            except:
                pass
