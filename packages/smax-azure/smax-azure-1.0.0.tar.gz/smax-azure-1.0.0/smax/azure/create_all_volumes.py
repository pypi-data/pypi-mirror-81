...
def run_me(command: List[str], work_dir: str = ".") -> object:
    outputs = []
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        "--smax-ns",
        dest="smax_ns",
        help="smax k8s namespace",
        type=str,
        default="itsma-azfile",
    )
    parser.add_argument(
        "-an",
        "--azure-secret-ns",
    ...
    parser.add_argument(
        ...
        "--storage-a-name",
        dest="ssa",
        help="azfile storage for generic",
        type=str,
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-ccrt",
        "--cdf-azfile-secret",
        dest="cdf_azfile_secret",
        help="cdf azfile secret name",
        type=str,
    ...
    parser.add_argument(
        "-scrt",
        "--smax-azfile-secret",
        dest="smax_azfile_secret",
        help="smax azfile secret name",
        type=str,
        default="azure-secret",
    )
...
for pv in all_volumes:
    ...
    yaml_file_path = p.joinpath(pv.name)
    with yaml_file_path.open("w") as f:
        f.write(real_yaml)
    print("creating use yaml => ", yaml_file_path)
    run_me(["kubectl", "create", "-f", yaml_file_path])
