...
def localize_rois(rois_data_source, psf, initial_estim=None, constants=None):
    """
    ...
    with EstimQueue(psf,batchSize=1024) as queue:
        ...
        queue.WaitUntilDone()
        r = queue.GetResults(getSampleData=True)
...
class Localizer2D:
    ...
    def process(self, fn, cfg, output_file=None, progress_cb=None):
        ...
        with Context() as ctx:
            ...
            if not progreport(f'Fitting 2D Gaussian to {numrois} ROIs (including PSF sigma) 1/2'):
                return None
            
            psf = gaussian.CreatePSF_XYIBg(roisize, spotDetectSigma, True)
            qr = localize_rois(rois_output_fn, psf)
            framenum = qr.ids

            if not progreport(f'Fitting 2D Gaussian to {numrois} ROIs (including PSF sigma) 2/2'):
                return None
            ...
            qr_sigma = localize_rois(rois_output_fn, psf_sigma, initial_estim=estim)
            ...
            ds = ds[ds.data.chisq<chiSqThreshold]
            ...
            if not progreport('Fitting 2D Gaussian to ROIs (fixed sigma)'):
                return None
            psf = gaussian.CreatePSF_XYIBg(roisize, sigma=None, cuda=True)
            roi_sigmas = self.sigma[framenum]
            r = localize_rois(rois_output_fn, psf, constants=roi_sigmas)
            ...
            if maxChiSquare is not None:
                ...
                self.unfilteredChisq = r.chisq*1
                r.Filter(r.chisq<self.chiSqThreshold)
            ...
            ds = Dataset.fromQueueResults(r, imgshape, config=cfg, imagefile=fn, sigma=self.sigma)
    ...
    def plotSigmaFits(self):
        plt.figure()
        plt.hist(self.data.estim.sigma.T, bins=100,label=['Sigma X', 'Sigma Y'])
        plt.legend()
        
    def plotChiSquare(self):
        plt.figure()
        plt.hist(self.unfilteredChisq, bins=100, range=[0,self.chiSqThreshold*1.5], label='Chi-square values')
        plt.axvline(self.chiSqThreshold, label='Threshold')
        plt.legend()
        plt.xlabel('Chi-square value (sum((data-model)^2/model)')

    def plotSigmaTimeSeries(self):
