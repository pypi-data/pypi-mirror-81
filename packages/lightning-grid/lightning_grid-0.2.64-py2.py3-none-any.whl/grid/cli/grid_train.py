    ...
    if not grid_credential:
        ...
        if not _default_credential:
            ...
            Detected multiple credentials. Which would you like to use?
                ...
                grid train {entrypoint} --grid_credential [CREDENTIAL_ID]
...
def train(ctx, script: str, entrypoint: str, config: Optional[click.File],
    ...
    #  parsed automatically, i.e. everything before
    #  a -- character.
    script_arguments = list(script_args)

    if not gpus and not cpus:
        notice_str = click.style('NOTICE', fg='yellow')
        cpu_warning = f"""
        {notice_str}

        Neither CPU or GPU number was specified, so 1 CPU will be
        used as a default.
