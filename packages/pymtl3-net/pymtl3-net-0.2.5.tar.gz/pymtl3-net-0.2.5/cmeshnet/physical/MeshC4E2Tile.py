...
class MeshC4E2Tile( Component ):
  ...
  def construct( s, PacketType, PositionType ):
    ...
    s.exp_recv = [ RecvIfcRTL( PhitType ) for _ in range(4) ]
    s.exp_send = [ SendIfcRTL( PhitType ) for _ in range(4) ]

    s.feedthru_recv = [ RecvIfcRTL( PhitType ) for _ in range(4) ]
    s.feedthru_send = [ SendIfcRTL( PhitType ) for _ in range(4) ]
    ...
    s.exp_channels = [ Channel( PacketType ) for _ in range(4) ]
    ...
    for i in range(4):
      s.feedthru_send[i] //= s.feedthru_recv[i]

    @update
    def up_cmesh_tiles():
      for i in range(ncores):
        s.cores[i].recv.msg    @= s.router.send[i+8].msg
        s.cores[i].recv.en     @= s.router.send[i+8].en
        s.router.send[i+8].rdy @= s.cores[i].recv.rdy

        s.router.recv[i+8].msg @= s.cores[i].send.msg
        s.router.recv[i+8].en  @= s.cores[i].send.en
        s.cores[i].send.rdy    @= s.router.recv[i+8].rdy
      ...
      for i in range(4):
        s.exp_channels[i].recv[0].msg @= s.router.send[i+4].msg
        s.exp_channels[i].recv[0].en  @= s.router.send[i+4].en
        s.router.send[i+4].rdy        @= s.exp_channels[i].recv[0].rdy

        s.exp_send[i].msg             @= s.exp_channels[i].send[0].msg
        s.exp_send[i].en              @= s.exp_channels[i].send[0].en
        s.exp_channels[i].send[0].rdy @= s.exp_send[i].rdy

        s.router.recv[i+4].msg        @= s.exp_channels[i].send[1].msg
        s.router.recv[i+4].en         @= s.exp_channels[i].send[1].en
        s.exp_channels[i].send[1].rdy @= s.router.recv[i+4].rdy

        s.exp_channels[i].recv[1].msg @= s.exp_recv[i].msg
        s.exp_channels[i].recv[1].en  @= s.exp_recv[i].en
        s.exp_recv[i].rdy             @= s.exp_channels[i].recv[1].rdy
