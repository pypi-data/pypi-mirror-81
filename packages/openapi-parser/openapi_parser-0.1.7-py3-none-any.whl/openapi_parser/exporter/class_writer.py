...
from openapi_parser.model import ModelClass, ModelSchema, HavingPath, HavingExtendedDescription
from openapi_parser.parser import EnumFilter
...
class ClassWriter(Exporting, AttributeWriter, Writer):
    def dump_class_description(self, cls: HavingExtendedDescription, *, path_cls: Optional[HavingPath] = None, cls_all_required_properties: List[str] = None) -> Iterator[str]:
        ...
        if (path_cls is None and isinstance(cls, HavingPath)):
            path_cls = cls
        
        yield self.TRIPLE_QUOT
        
        if (cls.summary is not None):
            yield from cls.summary.splitlines()
            yield
        ...
        if (path_cls is not None):
            yield f"Class {path_cls.pretty_path}"
            yield f"Schema {path_cls.path}"
        yield self.TRIPLE_QUOT
        yield
    
    def dump_enum(self, schema: ModelSchema, enum_filter: EnumFilter) -> Iterator[str]:
        cls_name = self.object_valid_name_filter(self.class_name_pretty(enum_filter))
        
        self.export(cls_name)
        yield f'class {cls_name}(Enum):'
        with self.indent():
            yield from self.dump_class_description(schema, path_cls=enum_filter)
