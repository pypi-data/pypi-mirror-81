...
from cognite.geospatial._util import plot_geometry
...
class SpatialObject(FullSpatialItemDTO, Geometry):
    ...
    def plot(self, attributes=None, label=None):
        plot = None
        if attributes is None:
            geometry = self.coverage(dimensional_space="2d", geometry_format="wkt")
            plot = plot_geometry(geometry, label=label or "Coverage")
        else:
            values = self.client.get_attributes(attributes=attributes, id=self.id)
            ...
            for name in values:
                ...
                if _is_geometry(layer_attribute):
                    geometry = wkt.loads(values[name])
                    if plot is None:
                        plot = plot_geometry(geometry, label=label or name)
                    else:
                        plot = plot * plot_geometry(geometry, label=label or name)
                elif _is_number(layer_attribute):
                    points.append(values[name])

            if len(points) % 2:
                geometry = LineString([(points[i], points[i + 1]) for i in range(0, len(points), 2)])
                if plot is None:
                    plot = plot_geometry(geometry, label=label or "Attributes")
                else:
                    plot = plot * plot_geometry(geometry, label=label or "Attributes")
