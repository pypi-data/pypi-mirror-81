...
class HARMONI(Instrument):
    ...
    def __init__(self, psf=None, lsf=None, pixscale=cube_default_xy_step, lsf_fwhm=None):
        if pixscale is not None:
            self.xy_step = self.cube_default_xy_step =  pixscale / 1.0e3    # @fixme deal with units
        else:
        ...
        if lsf is None:
            if lsf_fwhm is None:
                raise TypeError('Harmoni lsf_fwhm required')
            lsf = GaussianLineSpreadFunction(fwhm=lsf_fwhm)
