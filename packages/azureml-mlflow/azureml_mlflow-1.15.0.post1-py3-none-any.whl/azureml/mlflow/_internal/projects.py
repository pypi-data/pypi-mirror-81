...
def _azureml_arg_format(mlflow_command):
    '''
    Transforms a command string from mlflow into a format ingestable by AzureML ScriptRunConfig
    The command string is validated by mlflow so assume parameters are valid type
    Based on existing support for project workflows, we can only expect a single command string
    for each runtime call of mlflow.projects.run
    :param mlflow_command:
    :type mlflow_command: list[str]
    mlflow_command can look like:
    :rtype commands: dict containing target script and params of the form {script: "file.py", params: [[str]]}
    '''
    # possible mlflow_command
    # command: "python train.py -r {regularization} {data_file}"
    # command: "python validate.py {data_file}"
    # command: "python train.py arguments.py --alpha 0.5"

    invalid_args = ["python", "\\"]
    script, parameters = None, []
    mlflow_command = mlflow_command[0].split()
    for arg in mlflow_command:
        # ignore invalid arguments
        if arg in invalid_args:
            continue
        # pick first file as script and other files are treated as args
        if arg.endswith(".py") and script is None:
            script = arg
        else:
            # Follow cases should be handled here
...
def _load_azure_workspace():
    ...
    if isinstance(def_store, AzureMLRestStore):
        # passing None for location because service context retrieves the region
        workspace = Workspace._from_service_context(def_store.service_context, None)
        return workspace
    else:
...
class AzureMLProjectBackend(AbstractBackend):
    ...
    def run(self, project_uri, entry_point, params,
        ...
        script, params = _azureml_arg_format(command_args)
        ...
        src = ScriptRunConfig(source_directory=work_dir, script=script, arguments=params)
        ...
        if compute and compute != _LOCAL and compute != _LOCAL.upper():
            ...
            remote_environment.register(workspace=workspace)
            cpu_cluster = _load_compute_target(workspace, backend_config)
            src.run_config.target = cpu_cluster.name
            src.run_config.environment = remote_environment
        else:
