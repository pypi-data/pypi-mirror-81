...
def discriminator_decoder(discriminator_key: str, mappings: Dict[str, Type[T]]) -> Callable[[Dict[str, Any]], T]:
    lst = ', '.join(f"'{t}'" for t in mappings.keys())
    def decoder(data: Dict[str, Any]) -> T:
        if (not isinstance(data, dict)):
            raise ValueError(f"A dict-like object is expected to decode any of [ {lst} ], got {type(data)}")
        elif (discriminator_key not in data):
            raise ValueError(f"Discriminator field '{discriminator_key}' was not presented in the body: '{data}'")
        elif (data[discriminator_key] not in mappings):
            raise ValueError(f"Discriminator field '{discriminator_key}' has invalid value '{data[discriminator_key]}'")
        else:
