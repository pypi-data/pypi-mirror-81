...
class GoogleSigner(Signer, GoogleKMSInvoker):
    ...
    def kms_key(self):
        if not getattr(self, '_kms_key', None):
            self._kms_key = self._google_kms.get_public_key(self._get_resource_id())
        return self._kms_key

    @property
    def public(self):
        if not getattr(self, '_public_key', None):
            self._google_key = self._google_kms.get_public_key(self._get_resource_id())
