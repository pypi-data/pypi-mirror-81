...
The event is taking one event argument with the value of the app state and if it returns a value that is not `None` then
this value will be stored/saved.


application flow change events
------------------------------

The name of the event method that gets fired when the app want to change the flow (via the method
:meth:`~MainAppBase.change_flow`) depends on the new flow id and gets determined by the function
:func:`flow_event_name`.

If the event handler exists and does return `True` for to confirm the flow change then flow id and path will be changed
accordingly.

If the flow-specific event call results in a boolean `False` or `None` then a more generic event
:meth:`~MainAppBase.on_flow_change` will be called.

If also the generic event returns `False` then the action of the new flow id will be searched within
:data:`ACTIONS_CHANGING_FLOW_WITHOUT_CONFIRMATION` and if not found then the flow change will be rejected and
:meth:`~MainAppBase.change_flow` returns `False`

If a confirmed flow change is resulting in a `'focus'` flow action then the event `on_flow_widget_focused` will fired
and can be used by the GUI framework for to set the focus to the widget associated to the focus flow id.
...
class MainAppBase(ConsoleApp, ABC):
    ...
    def on_app_init(self):
        """ default/fallback event handler. """
    ...
    def on_font_size_change(self, font_size_lit: str, _event_kwargs: Dict[str, Any]) -> bool:
        """ font size app state change event handler.

        :param font_size_lit:   font size as string.
        :param _event_kwargs:   unused event kwargs.
        :return:                True for to confirm change of flow id.
        """
        self.vpo(f"MainAppBase.on_font_size_change {font_size_lit}")
        self.change_app_state('font_size', float(font_size_lit))
        return True
    ...
    def change_flow(self, new_flow_id: str, popups_to_close: Sequence = (), **event_kwargs) -> bool:
        ...
        :param event_kwargs:    optional args to pass additional data or info onto and from the event handler.
                                ...
                                info pass onto event handler (e.g. from :attr:`~ae.kivy_app.FlowButton.tap_kwargs`
                                or :attr:`~ae.kivy_app.FlowToggler.tap_kwargs`):
