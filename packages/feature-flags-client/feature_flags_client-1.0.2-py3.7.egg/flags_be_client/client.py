...
def push_metrics(flag_name, identifier, gated):
    ...
    gated = bool(gated)
    message = (
        f"feature_flags.[flag_name={flag_name},identifier={identifier}]gated:{gated}|c"
    )
...
class FeatureFlagClient:
    ...
    def get_flag_file_content(self, bucket_path):
        response = requests.get(
            f"https://stitch-feature-flags.s3.eu-central-1.amazonaws.com/{bucket_path}/ff_test.json"
        )
    ...
    def is_enabled(self, flag_name, identifier=""):
        ...
        for flag in flags_list:
            if flag["name"] == flag_name:
                return self.check_is_visible(flag, identifier)
