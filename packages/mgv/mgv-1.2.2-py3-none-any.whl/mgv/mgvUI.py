...
class MgvActionWindow(QtWidgets.QWidget):
    ...
    def exe(self, node_version, action):
        if node_version:
            type_updated = mgvWrapper.getType(node_version.node.graph.getProject().getName(),
                                              uuid=node_version.node.getType().uuid)
            if type_updated is not None:
                node_version.node.type._copy(type_updated)
            ...
            node_version.getProject().refreshHead()
        for task in self.tasks:
            task.widget.hide()
        ...
        if task.widget:
            ...
            if value == 2:  # finished
                ...
                if task.node_version:
                    ...
                    if node.user != node.graph.user and len(node.port.split(':')) > 1:
...
MGVNODETYPEVERSION :\t\tNode type version
