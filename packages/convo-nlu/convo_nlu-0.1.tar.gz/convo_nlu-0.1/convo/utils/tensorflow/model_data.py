...
from convo.utils.tensorflow.constants import BALANCED, SEQUENCE
...
class ConvoModelData:
    ...
    def __init__(
        ...
        """
        Initializes the ConvoModelData object.
    ...
    def add_features(self, key: Text, features: List[np.ndarray]):
        ...
        if key in self.data:
            raise ValueError(f"Key '{key}' already exists in ConvoModelData.")
    ...
    ) -> Tuple["ConvoModelData", "ConvoModelData"]:
    ...
    def get_signature(self) -> Dict[Text, List[FeatureSignature]]:
        """Get signature of ConvoModelData.
    ...
    def _split_by_label_ids(
        ...
        for label_id in unique_label_ids:
            ...
            label_data.append(
                ConvoModelData(self.label_key, self._data_for_ids(data, matching_ids))
            )
        return label_data
    ...
    def _check_label_key(self):
        if self.label_key is not None and (
            ...
            raise ValueError(f"Key '{self.label_key}' not in ConvoModelData.")
    ...
    def _convert_train_test_split(
        ...
        return (
            ConvoModelData(self.label_key, data_train),
            ConvoModelData(self.label_key, data_val),
        )
