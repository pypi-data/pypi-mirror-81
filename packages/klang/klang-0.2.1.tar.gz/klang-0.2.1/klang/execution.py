...
from klang.block import input_neighbors, output_neighbors, Block
from klang.graph import graph_matrix, topological_sorting


def traverse_network(blocks: Iterable[Block]) -> Tuple[List[Block], Set[Tuple[Block, Block]]]:
...
def determine_execution_order(blocks: Iterable[Block]) -> List[Block]:
...
def execute(blocks: Iterable[Block]):
    """Execute blocks."""
    for block in blocks:
        block.update()


def print_exec_order(execOrder: Iterable[Block]):
