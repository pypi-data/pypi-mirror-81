...
class cpp(object):
    ...
    def day_eqm(self,acu=False,f_cal=None,f_apl=None):
        ...
        if acu==True:
            print("acotando numeros cercanos a cero")
        ...
        for mon in meses:
            ...
            datos_his=dm_f
            datos_pro=da_f
            datos_obs_q=do_f.quantile(quantiles,dim='time').astype('float32')
            datos_his_q=dm_f.quantile(quantiles,dim='time').astype('float32')
            datos_pro_q=da_f.quantile(quantiles,dim='time').astype('float32')
            ...
            deltas=datos_his_cor.quantile(quantiles,dim='time').astype('float32')-datos_his.quantile(quantiles,dim='time').astype('float32')
        ...
        if acu==True:
            ...
            cor=cor.where(self.data['do'].isel(time=0)>-888) #Para tener solo los datos en lugares donde habia observados
            #cor=cor.where(~(cor<0.001),0)
        else:
            cor=datos_pro_cor2
            cor=cor.where(self.data['do'].isel(time=0)>-888) #Para tener solo los datos en lugares donde habia observados
        #NOTA: LAS ADVERTENCIAS SALEN DEBIDO A QUE NO HAY DATOS EN OCEANO (POR PARTE DEL OBSERVADO)
        print('Terminado EQM diario')
        return correct_mon(cor,self.data['da'],self.data['do'],self.data['dm'],acu=acu)
