...
def celeb_recognition(image_path, ann_index, celeb_mapping_dict, save_img_output=False, url=False):
	if url:
	...
	pred, img_out = get_celeb_prediction(img, ann_index, celeb_mapping_dict)
	if pred is not None:
		if save_img_output:
