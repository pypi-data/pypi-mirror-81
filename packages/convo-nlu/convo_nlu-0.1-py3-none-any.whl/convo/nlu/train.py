...
from convo.nlu.training_data.loading import load_data_from_endpoint
...
async def train(
    nlu_config: Union[Text, Dict, ConvoNLUModelConfig],
    ...
    **kwargs: Any,
    ...
    if not isinstance(nlu_config, ConvoNLUModelConfig):
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `convo.nlu.train` directly is no longer supported. Please use "
        "`convo train` to train a combined Core and NLU model or `convo train nlu` "
