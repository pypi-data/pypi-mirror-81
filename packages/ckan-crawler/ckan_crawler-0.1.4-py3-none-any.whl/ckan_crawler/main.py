...
def write_know_resources(item, status):
    ...
    with p_know_resources.open("w") as f:
        json.dump(know_resources, f, indent=2, sort_keys=True)
...
def find_resources_to_donwload(all_resources):
    ...
    for item in all_resources:
        ...
        if item_resource_id in know_resources:
            item_resource_last_modified = item["resource_last_modified"]
            know_resource_resource_last_modified = know_resources[item_resource_id]["resource_last_modified"]

            if not item_resource_last_modified:
                continue
            
            elif not know_resources and item_resource_last_modified:
                updated_items.append(item)

            elif parse_date(know_resource_resource_last_modified) < parse_date(item_resource_last_modified):
                updated_items.append(item)
