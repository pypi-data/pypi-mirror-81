...
def load(
    ...
    if config is not None:
        try:
            file_config = convo.utils.io.read_config_file(config)
...
def _load_from_dict(config: Dict, **kwargs: Any) -> "ConvoNLUModelConfig":
    if kwargs:
        config.update(kwargs)
    return ConvoNLUModelConfig(config)
