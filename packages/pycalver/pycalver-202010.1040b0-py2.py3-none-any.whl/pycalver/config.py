...
def _validate_version_with_pattern(current_version, version_pattern,
    ...
    if is_new_pattern:
        ...
        if invalid_chars:
            errmsg = (
                'Invalid character(s) \'{0}\' in pycalver.version_pattern = "{1}"'
...
def _set_raw_config_defaults(raw_cfg):
    if 'current_version' in raw_cfg:
        if not isinstance(raw_cfg['current_version'], str):
            err = 'Invalid type for pycalver.current_version = {0}'.format(
                raw_cfg['current_version'])
            raise TypeError(err)
    else:
    ...
    if 'version_pattern' in raw_cfg:
        if not isinstance(raw_cfg['version_pattern'], str):
            err = 'Invalid type for pycalver.version_pattern = {0}'.format(
                raw_cfg['version_pattern'])
            raise TypeError(err)
    else:
