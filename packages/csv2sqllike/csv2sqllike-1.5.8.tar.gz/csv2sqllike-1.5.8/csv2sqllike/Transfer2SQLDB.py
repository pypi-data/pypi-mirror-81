...
class Transfer2SQLDB(object):
    ...
    def delete_table(self, table_name: str) -> None:
        tmp_connection = pymysql.connect(**self.__database_info)
        try:
    ...
    def get_tables(self) -> list:
        ...
        try:
            with tmp_connection.cursor() as cursor:
                ...
                tmp_list = list(x["Tables_in_{}".format(self.__db.db.decode("utf-8"))] for x in cursor.fetchall())
        finally:
            tmp_connection.close()
        return tmp_list
    ...
    def create_table(self, table_name: str, input_pseudosql_or_df: pd.DataFrame, backup=False, keys=None) -> None:
        ...
        try:
            with tmp_connection.cursor() as cursor:
                ...
                self.__write_meta_table_meta_info(table_name, cursor)
    ...
    def bring_data_from_table(self, table_name: str) -> pd.DataFrame:
        ...
        try:
            with tmp_connection.cursor() as cursor:
                ...
                tmp_dtype_df = pd.DataFrame(self.__cursor.fetchall())
        ...
        try:
            with tmp_connection.cursor() as cursor:
                if exclude_history is False:
                    self.__write_meta_table_meta_info(table_name, cursor)
                self.__insert_data(table_name, input_pseudosql_or_df, cursor)
            tmp_connection.commit()
        finally:
            tmp_connection.close()
    ...
    def __check_if_exist(self, table_name: str, cursor) -> bool:
        cursor.execute("show tables")
        tmp_list = list(x["Tables_in_{}".format(self.__db.db.decode("utf-8"))] for x in cursor.fetchall())
    ...
    def __write_meta_table_meta_info(self, table_name: str, cursor) -> None:
        if not self.__check_if_exist("table_history", cursor):
            self.__make_table_history_table(cursor)

        tmp_now = datetime.now()
        if self.__check_if_exist(table_name, cursor):
            tmp_action = "modify"
        else:
            tmp_action = "create"
