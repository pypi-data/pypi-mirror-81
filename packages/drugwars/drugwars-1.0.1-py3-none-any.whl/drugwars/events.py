...
def get_mugged(p):
    if 1 == randint(1, 10):
        clear()
        print(SingleTable([["You got mugged!! You lost " + str(p.money - int(round_down(p.money * 0.80)))]]).table)
...
def display_pricing_screen(p):
    ...
    while True:
        ...
        money_table = lambda : [
            ['Debt: ' + str(int(round_down(p.shark.balance))), 'Wallet: ' + str(int(round_down(p.money))), 'Guns: ' + str(p.guns)]
        ]
