...
from qary.constants import ES_HOST, ES_PORT, ES_INDEX, ARTICLES_URL
...
try:
    ...
    log.error("Failed to launch Elstcisearch")
...
def add_parsed_document_to_elasticsearch(sections, index=ES_INDEX):
    ...
    if not exists:
        log.info(f"Loading article with page id {pageid} to ElasticSearch...")
...
def in_index(article_id=None, title=None, index=ES_INDEX):
    ...
    >>> in_index(article_id=123, index='')
    False
    >>> in_index(article_id=31491224, index='')
    True
    >>> in_index(title='Trevor Noah', index='')
    True
    """
    if title:
...
def load_article_from_wikipedia(pageid=None, title=None):
    page = wiki_wiki.page(pageid or title)
...
def parse_and_index_wikipedia_article(title='Trevor Noah', index=ES_INDEX):
    ...
    records = build_elasticsearch_record(page, sections)
    add_parsed_document_to_elasticsearch(sections=records, index=index)
