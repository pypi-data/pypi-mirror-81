...
class BaseDataConnection(ABC):
    ...
    def _download_obm_json(self) -> dict:
        ...
        if self.type == DataConnectionTypes.S3 and self._obm:
        ...
        elif self.type == DataConnectionTypes.FS and self._obm:
            # note: fetch all OBM file part names
            obm_json_url = f"{self._wml_client.wml_credentials['url']}/v2/asset_files/auto_ml/{self.location.path.split('/auto_ml/')[-1]}/{self._run_id}/data/obm/obm.json"
            file_response = requests.get(obm_json_url,
            ...
            if file_response.status_code != 200:
                raise ApiRequestFailure(u'Failure during {}.'.format("downloading obm json"), file_response)
            ...
            raise NotImplementedError("OBM is not supported for WML Server yet.")
