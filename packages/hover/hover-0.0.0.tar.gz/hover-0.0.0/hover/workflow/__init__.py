...
from hover.annotation import CustomTextcatSession, LabelingFunctionSession
...
class LabelingFunctionPopulation(object):
    ...
    def __init__(self, candidates, incumbents, **kwargs):
        ...
        self.set_incumbents(incumbents)
    ...
    def set_incumbents(self, incumbents):
    ...
    def deduplicate(self):
        ...
        """
        self.set_incumbents(deduplicate_lfs(self.__incumbents))
        self.set_candidates(
            deduplicate_lfs(
                self.__candidates,
                lookup_set=set([lf_string_representation(_lf) for _lf in self.__incumbents]),
            )
        )
    ...
    def add_incumbents(self, incumbents):
        """
        Manually add incumbents.
        """
        self.__incumbents += incumbents
        self.__statistics_outdated = True
...
class Automated(object):
    ...
    def __init__(self, dataset, lf_population, model_module_name):
        """
        Start an automated workflow.
        :param dataset: workflow.Dataset object.
        :param lf_population: worklow.LabelingFunctionPopulation object.
    ...
    def get_loader(self, key, vectorizer, smoothing_coeff=0.0):
        ...
        if key == "train":
            ...
            texts, labels = (
                self.dataset.dfs[key]["text"].tolist(),
                np.array(
                    self.dataset.dfs[key][module_params.ENCODED_LABEL_KEY].tolist()
                ),
            )
    ...
    def compute_text_to_2d(self, method, **kwargs):
        ...
        fit_texts = []
        for _key in subset:
            fit_texts += self.dataset.dfs[_key]["text"].tolist()
    ...
    def lf_rerank(self):
        ...
        self.lf_population.set_incumbents(incu_to_incu + cand_to_incu)
        self.lf_population.set_candidates(incu_to_cand + cand_to_cand)
    ...
    def lf_refill(
        ...
        analysis_result = lf_analysis(
            raw_candidates, df_train=None, df_dev=self.dataset.dfs["dev"]
        )
        ...
        self.lf_population.add_candidates(accepted_lfs)
