...
def _get_path_sect_keys(mapping, keys=[PATH_KEY]):
    ...
    """
    return [k for k, v in list(mapping.items()) if bool(set(keys) & set(mapping[k]))]
...
def _populate_paths(object, schema, check_exist):
    ...
    for ps in path_sects:
        templ = s[ps][PATH_KEY]
        try:
            populated = templ.format(**dict(list(object.items())))
...
def get_project_outputs(project, schema):
    ...
    for ps in path_sects:
        ...
        for p in PATH_LIKE:
            try:
                res[ps][p] = s[ps][p].format(**dict(list(project.items())))
            except Exception as e:
