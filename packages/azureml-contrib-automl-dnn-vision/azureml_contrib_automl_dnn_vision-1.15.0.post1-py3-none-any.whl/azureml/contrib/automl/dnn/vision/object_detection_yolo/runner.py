...
def _find_cfg_and_weights(settings):
    """Find file paths for cfg and model weights based on model size

    :param settings: Dictionary with all training and model settings
    :type settings: Dictionary
    :return: File paths for model_cfg and model_weights
    :rtype: String and String
    """
    ...
    weights = model_name[:-1] + YoloParameters.DEFAULT_MODEL_VERSION + size + '.pth'

    current_file_path = os.path.dirname(__file__)
    model_cfg = check_file(os.path.join(current_file_path, YoloLiterals.MODELS, cfg))
    model_weights = check_file(os.path.join(current_file_path, YoloLiterals.WEIGHTS, weights))

    return model_cfg, model_weights
...
def run(automl_settings):
    ...
    # Update cfg and model weights based on model_size
    cfg, weights = _find_cfg_and_weights(settings)
