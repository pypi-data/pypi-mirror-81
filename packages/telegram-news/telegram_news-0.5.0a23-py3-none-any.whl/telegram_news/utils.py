...
def extract_video_config(video_full_name, thumb_full_name, thumb_name):
    ...
    if success:
        ...
        if os.path.getsize(thumb_full_name) > MAX_THUMB_SIZE:
            print("you are right", MAX_THUMB_SIZE * 100000 / os.path.getsize(thumb_full_name))
            encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), int(MAX_THUMB_SIZE * 100000 / os.path.getsize(thumb_full_name))]
            result, encimg = cv2.imencode('.jpg', image, encode_param)
            decimg = cv2.imdecode(encimg, 1)
            print(decimg)
            if result:
                print("but anyway, i will fix it!")
