...
class BigQueryWrapper(object):
    ...
    def __init__(self, client: bigquery.Client, bqstorage_client: BigQueryStorageClient = None) -> None:
    ...
    def from_params(cls, project: str, auth_file: Optional[str] = None, setup_bqstorage: bool = True):
        if auth_file:
            ...
            bqstorage_client = BigQueryStorageClient.from_service_account_json(auth_file) if setup_bqstorage else None
        else:
            client = bigquery.Client(project=project)
            bqstorage_client = BigQueryStorageClient() if setup_bqstorage else None
    ...
    def to_table_from_gcs(self, source_gcs_uri: str, table_reference: str, source_type: str = 'csv',
        ...
        project, dataset, table = self._parse_table_reference(table_reference)
        destination_table = self.client.dataset(dataset).table(table)
        # Create job config
