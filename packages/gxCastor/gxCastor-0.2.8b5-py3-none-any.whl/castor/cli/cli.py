...
from castor.display.attributed_str import bold
import castor.display.attributed_str
    ...
    with open(filename, 'r') as fp:
        ...
        if output != -1:
            ...
            with open(sample_file, 'w') as sp:
                for line in indexes:
                    print(line, file=sp)
            rp(castor.display.attributed_str.clear(f"{sample_file} Created!"))
...
def csv(filename: str = typer.Option(None, '--file', '-f'),
    ...
    with open(filename, 'r') as csvfile:
        ...
        rp(castor.display.attributed_str.bcyan(f"{filename}: {len(rows) - 1} row(s)"))
        rp(castor.display.attributed_str.bcyan(f"Sample {k} items"))
        rp(castor.display.attributed_str.bred('=' * 25))
        console.print(table)
        rp(castor.display.attributed_str.bred('=' * 25))
...
@app.command('xls')
def excel(filename):
    rp(castor.display.attributed_str.bred("Will Implement in future version"))
...
def locker(name: str = typer.Option("hash", '-n', '--name', help="Give lock name")):
    ...
    if os.path.exists(lock_file):
        ...
        rp(castor.display.attributed_str.clear("Verifying file..."))
        ...
        rp(castor.display.attributed_str.bred(f"{len(diff['deleted'])} files deleted"))
        for item in diff['deleted']:
            rp(castor.display.attributed_str.bred(f"----{item}"))
        rp(castor.display.attributed_str.byellow(f"{len(diff['modified'])} files modified"))
        for item in diff['modified']:
            rp(castor.display.attributed_str.byellow(f"-+-+{item}"))
        rp(castor.display.attributed_str.bred(f"{len(diff['new'])} files created"))
        for item in diff['new']:
            rp(castor.display.attributed_str.bgreen(f"++++{item}"))
