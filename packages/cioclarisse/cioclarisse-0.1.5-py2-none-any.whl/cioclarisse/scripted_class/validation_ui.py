...
class ValidationWindow(ix.api.GuiWindow):
    ...
    def __init__(self, warnings, notices, dry_run):
        ...
        if self.dry_run:
            spacer_width = BOTTOM_BUT_WIDTH*2
    ...
    def _populate(self):
        ...
        """
        warnings = ["[WARN]{}".format(w) for w in self.warnings]
        notices = ["[INFO]{}".format(n) for n in self.notices]
...
def proceed( warnings, notices, dry_run=False):
    """
    ...
    win = ValidationWindow( warnings, notices, dry_run)
