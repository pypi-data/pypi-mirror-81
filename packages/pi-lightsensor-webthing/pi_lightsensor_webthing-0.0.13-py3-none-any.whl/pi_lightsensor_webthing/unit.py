...
[Service]
Type=simple
ExecStart=$entrypoint --command listen --port $port --gpio $gpio_number --name $name
...
def register(packagename, entrypoint, port, name, gpio_number):
    unit = UNIT_TEMPLATE.substitute(packagename=packagename, entrypoint=entrypoint, port=port, name=name, gpio_number=gpio_number)
