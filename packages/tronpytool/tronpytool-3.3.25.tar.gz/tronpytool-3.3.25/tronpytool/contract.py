...
class ContractFunctions:
    ...
    def ___get_abi_by_name(self, func_name: str) -> list:
        return filter_by_name(func_name, self.abi)

    def ___print_functions(self):
        for func in self._functions:
            if func['type'] == 'function':
                if func['stateMutability'] in ['pure', 'view']:
                    self.___p('read', func)
                if func['stateMutability'] in ['nonpayable']:
                    self.___p('write', func)

            if func['type'] == 'constructor':
                self.___p('constructor', func)
            print("=====")
            print(func)

    def ___p(self, typecontrol, func):
        b = "{} {}({}) {}"
        input_space = ""
        oput_space = ""
