...
class TD_IPSO_3306_02(CoAPTestCase):
    """
testcase_id: TD_IPSO_3306_02
uri: http://openmobilealliance.org/iot/lightweight-m2m-lwm2m
configuration: LWM2M_CFG_01
objective: 
  - Setting the writable resources of object 3306 (IPSO Actuation) Instance 0 using JSON data format (11543)
...
pre_conditions: 
  ...
  - The Client supports IPSO Actuation (ID:3306) with mandatory resources. 
sequence:
  - step_id: 'TD_IPSO_3306_02_step_01'
    type: stimuli
    node: lwm2m_server
    description:
      - 'LwM2M server sends a WRITE request (CoAP PUT) on Actuation object instance'
      - - Type = 0 (CON)
        - Code = 3 (PUT)

  - step_id: 'TD_IPSO_3306_02_step_02'
    type: check
    description:
      - 'Sent PUT request contains'
      - - Type=0 and Code=3
        - Non-empty payload = a serialized representation of Actuation object 
        - content-format=application/vnd.oma.lwm2m+json
        - URI-Path option= 3306/0

  - step_id: 'TD_IPSO_3306_02_step_03'
    type: check
    description:
  ...
  - step_id: 'TD_IPSO_3306_02_step_04'
    type: verify
    node: lwm2m_server
    description:
    ...
    def run(self):
        ...
        self.match('server', CoAP(type='con', code='put', opt=self.uri('/3306/0')), 'fail')
