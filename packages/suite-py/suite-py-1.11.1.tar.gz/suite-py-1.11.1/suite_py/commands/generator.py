...
class Generator:
    ...
    def _git_operations(self, git, autobranch, autopush, branch_name, message):
        if autobranch:
            logger.info(f"{git.get_repo()}: creo il branch {branch_name}")
            ...
            if git.current_branch_name() != "master":
                ...
                logger.warning(
                    f"{git.get_repo()} si trova su master, non eseguo il push automatico"
                )
...
def _configure_branch(git, template_type, autobranch, autopush):
    logger.info(f"{git.get_repo()}: Sincronizzo il repo con il remote")
    if autobranch:
