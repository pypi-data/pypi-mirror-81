...
from layeris.utils.conversions import convert_uint_to_float, convert_float_to_uint
import bgeditor.common.utils as utils
class Matrix:
            ...
            if len(self.arr_color)>0:
                for color in self.arr_color:
                    img_layer = Image.fromarray(
                        utils.change_color_alpha(np.asarray(template_a), color))
