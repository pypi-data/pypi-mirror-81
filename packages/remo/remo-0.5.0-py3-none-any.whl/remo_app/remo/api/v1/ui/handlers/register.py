...
class Register(viewsets.GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            users.create_or_update_superuser(email=email, username=username, fullname=fullname, company=company, allow_marketing_emails=allow_marketing_emails)
        except Exception as err:
            return Response({'error': f'failed to update user details: {err}'}, status=status.HTTP_400_BAD_REQUEST)

        ok, _, _ = license.register_user(email=email, username=username, fullname=fullname, company=company,
