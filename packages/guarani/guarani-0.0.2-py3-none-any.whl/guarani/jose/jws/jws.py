...
from guarani.jose.exceptions import InvalidJWSSerializationError, InvalidKeyError
from guarani.jose.jwk import JsonWebKey
from guarani.jose.jws.header import JsonWebSignatureHeader
from guarani.webtools import base64url_decode, base64url_encode, json_dumps, json_loads
...
class JsonWebSignature:
    ...
    def serialize(self, key: JsonWebKey) -> bytes:
        ...
        :raises InvalidKeyError: The provided key is invalid.
    ...
    def deserialize(
        ...
        :raises InvalidJWSSerializationError: The provided JWS token is invalid.
        :raises InvalidKeyError: The provided key is invalid.
        :raises InvalidSignatureError: Unmatching token signature and content.
        ...
        try:
            ...
            raise InvalidJWSSerializationError
        ...
        if algorithm:
            if jws_header.get("alg") != algorithm:
                raise InvalidJWSSerializationError(
                    "The provided algorithm does not match "
                    "the one on the header of the token."
                )
    ...
    def validate_key(cls, key: JsonWebKey, header: JsonWebSignatureHeader):
        ...
        if key.get("use"):
            if key.get("use") != "sig":
                raise InvalidKeyError("This key cannot be used to sign a JWS.")
