define([
    ...
    , 'nbextensions/visualpython/src/component/fileNavigation/helperFunction'
    , './state'
    , './renderer' // 렌더링 수정
    , 'nbextensions/visualpython/src/component/fileNavigation/executor'
], function ( requirejs, $, vpCommon, vpConst, sb, vpFuncJS,
              componentConstApi, helperFunction, FileNavigationState, FileNavigationRenderer, fileNavigationExecutorFuncList) {

    var { makeDom } = componentConstApi;
    var { makeKernelCurrentPath } = helperFunction;
    ...
    var vp_init = function(importPackageThis, type) {
        ...
        vpFuncJS.kernelExecuteV2(`%pwd`,(currentDirStr) => {
            ...
            fileNavigationState.setBaseFolderStr(rootFolderName);
            fileNavigationState.setRelativePathStr(rootFolderName);
            fileNavigationState.setBaseDirStr(slicedCurrentDirStr);
            ...
            if (notebookPath.indexOf("/") !== -1) {
                ...
                fileNavigationState.setNotebookPathStr(notebookPathStr);
            } else {
  
                fileNavigationState.setNotebookFolder(rootFolderName);
                fileNavigationState.setNotebookPathStr(slicedCurrentDirStr);
            }
    ...
    var vp_bindEventFunctions = function() {
        ...
        $('.directoryComponent-btn-up').click(() => {
            ...
            if ( fileNavigationState.getCurrentDirStr() === fileNavigationState.getNotebookPathStr() ) {
            ...
            executeKernelFromDirRightBody(dirObj, fileNavigationRenderer);
        });
