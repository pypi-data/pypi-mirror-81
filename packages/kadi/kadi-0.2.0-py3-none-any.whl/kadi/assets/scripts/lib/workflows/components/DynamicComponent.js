...
export default class DynamicComponent extends Rete.Component {
  constructor(flagSocket) {
    super('Bool');
  ...
  builder(node) {
    const out1 = new Rete.Output('flag', 'Bool', this.flagSocket);
    node.componentType = this.componentType;
    node.nodeDescription = 'Source data type Bool';

    return node
      .addControl(new CheckboxControl(this.editor, 'flag'))
      .addOutput(out1);
  }
