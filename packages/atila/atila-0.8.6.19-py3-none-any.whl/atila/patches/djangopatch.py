...
class Model (AbstractModel, models.Model):
    ...
    def cursoring (cls, *Qs, **filters):
        ...
        return (cls.db (cursor = True).select (cls)
                        .filter (*Qs, **filters))

    @classmethod
    def with_ (cls, alias, cte, cursor = False):
        return cls.db (cursor = cursor).with_ (alias, cte)

    @classmethod
    def fromcte (cls, alias, cursor = False):
        # followed by .with_ ()
        return cls.db (cursor = cursor).select (alias)

    @classmethod
    def alias (cls, tbl_alias, cursor = False):
        # followed by .join ()
        return cls.db (cursor = cursor).select (cls, tbl_alias)
