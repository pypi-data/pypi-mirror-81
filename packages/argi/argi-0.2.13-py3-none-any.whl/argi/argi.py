...
class argi:
  ...
  def clustering_description(self):
  ...
  def ts_visualization(self, lower, upper, start, stop, freq):
    ...
    # base = alt.Chart().mark_line().encode(x="index")
    # alt.layer( *[base.encode(y=col) for col in salida_df.columns if col!='index'], data=salida_df).interactive(bind_y=False).properties(
    #     width=1200,
    #     height=500
    # )
    ...
    for columna in lista[1:]:
      fig.add_scatter(x=salida_df['index'], y=salida_df[columna], mode='lines')
