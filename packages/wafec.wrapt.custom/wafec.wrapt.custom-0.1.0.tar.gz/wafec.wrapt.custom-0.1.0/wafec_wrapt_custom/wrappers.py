...
class WrapperTest(wrapt.ObjectProxy, ABC):
    ...
    def __getitem__(self, item):
        ...
        wrapper_data = self.notify_external(item, 'getitem', repr(value))
    ...
    def __getattr__(self, name):
        ...
        wrapper_data = self.notify_external(name, 'getattr', repr(value))
        return WrapperTest.will_wrap(value, wrapper_data=wrapper_data)
    ...
    def __next__(self):
        ...
        wrapper_data = self.notify_external(self._self_iter_idx, 'iter', repr(value))
    ...
    def notify_external(self, signature, event_name, value):
        if self._self_wrapper_data:
            ...
            self._self_wrapper_data.manager.notify(DataEvent(wrapper_data.signature, event_name, value))
            return wrapper_data
    ...
    def wrap_kwargs(kwargs, wrapper_data=None):
        for argname, argvalue in kwargs.items():
            kwargs[argname] = WrapperTest.will_wrap(argvalue, wrapper_data=wrapper_data)
        return kwargs
...
class WrapperData(object):
    ...
    def of(signature, wrapper_data):
        ...
        if wrapper_data.signature is not None:
            signature = wrapper_data.signature + ' ' + signature
