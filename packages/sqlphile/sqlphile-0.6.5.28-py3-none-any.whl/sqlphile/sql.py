...
class SQLComposer (SQL):
    ...
    def _join_select_fields (self):
        ...
        try:
            model_fks = self._model.get_fks ()
        except NotImplementedError:
            return

        select = []
        for fd in map (lambda x: x.strip (), self._feed ["select"].split (",")):
            a, *b = fd.split ()
            if a in model_fks:
                select.append (' '.join ([model_fks [a][0]] + b))
                continue
            if '__' in a:
                s = a.rfind ('__')
                fk_chain, colname = a [:s], a [s + 2:]
                if fk_chain in self._fk:
                    select.append (' '.join ([self._fk [fk_chain] + '.' + colname] + b))
                    continue
            select.append (fd)
        self._feed ["select"] = ", ".join (select)
