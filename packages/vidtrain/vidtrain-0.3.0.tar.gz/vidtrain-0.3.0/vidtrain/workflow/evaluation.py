...
class JunctionEvaluation(gui.backend.Panel, interface.WorkflowStep):
    ...
    def create_figures(self):
        if self.single_plots is None:
            self.single_plots = gui.backend.JunctionEvaluationPlots(
                master=self.single_stack_container, data=self.workflow_data.annotation, height=self.plot_heigth)
            self.combined_plots = gui.backend.CombinedEvaluationPlots(
    ...
    def create_compare(self):
        if self.single_plots_compare is None:
            self.single_plots_compare = gui.backend.CompareJunctionEvaluationPlots(
            ...
            self.combined_plots_compare = gui.backend.CompareCombinedEvaluationPlots(
    ...
    def _format_performance(self, pred=None, corr=None, threshold=0.1):
        if pred is not None and corr is not None:
            ...
            pr = (np.array(self.workflow_data.pred_med) > threshold).astype(np.float32)
            cor = (np.array(self.workflow_data.pred_corr) > threshold).astype(np.float32)
