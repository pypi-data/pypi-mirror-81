...
class TagPatchSchema(Schema, AttrsMixin, MacMixin):
    name = fields.Str(allow_none=True)
    zone_id = fields.Int(allow_none=True, validate=lambda x: x > 0)
    uuid = fields.UUID(allow_none=True)
    major = fields.Int(allow_none=True, validate=lambda x: x >= 0 < (2 ** 16))
    minor = fields.Int(allow_none=True, validate=lambda x: x >= 0 < (2 ** 16))
...
class ProxLinks(Schema):
    ...
    anchor = fields.URL(required=True)
