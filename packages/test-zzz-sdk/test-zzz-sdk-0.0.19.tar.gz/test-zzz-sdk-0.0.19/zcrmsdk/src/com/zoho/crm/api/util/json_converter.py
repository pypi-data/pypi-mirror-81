...
class JSONConverter(Converter):
    ...
    def is_not_record_request(self, request_instance, class_name, class_detail, instance_number, class_member_detail):
        ...
        if not skip_mandatory and self.common_api_handler.mandatory_checker is not None and self.common_api_handler.mandatory_checker:
        ...
        elif not skip_mandatory and lookup and self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_UPDATE:
            if len(primary_keys) > 0:
                error = {
                    Constants.FIELD: class_member_name,
                    Constants.TYPE: request_instance.__module__,
            ...
            if self.common_api_handler.category_method.upper() == Constants.REQUEST_CATEGORY_CREATE:
                if lookup:
                    if len(primary_keys) > 0:
                        error = {
                            Constants.FIELD: class_member_name,
                            Constants.TYPE: record_instance.__module__,
                            Constants.KEYS: str(primary_keys)
                        }
                        if instance_number is not None:
                            error[Constants.INSTANCE_NUMBER] = instance_number
