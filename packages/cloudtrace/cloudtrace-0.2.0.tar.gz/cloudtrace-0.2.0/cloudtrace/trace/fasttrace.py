...
def trace(targets: List[str], outfile: str, pps, proto, pid, waittime=30, timer='nano'):
    iface, src, nexthop = conf.route.route(targets[0])
    comp = None
...
def main():
    ...
    while args.cycles == 0 or cycle < args.cycles:
        ...
        trace(targets, filename, args.pps, args.proto, pid, waittime=args.wait, timer=args.timer)
        if args.remote:
