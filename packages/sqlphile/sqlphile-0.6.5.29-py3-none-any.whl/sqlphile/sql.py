...
class SQLComposer (SQL):
    ...
    def exclude (self, *Qs, **filters):
        return super ().exclude (*Qs, **self._resolve_fk (filters))

    def filter (self, *Qs, **filters):
        return super ().filter (*Qs, **self._resolve_fk (filters))
    ...
    def _resolve_fk (self, filters):
        try:
            model_fks = self._model and self._model.get_fks () or {}
        except NotImplementedError:
            return filters
        if not self._fk and not model_fks:
            return filters

        _filters = {}
        for fk in sorted (self._fk.keys (), key = lambda x: len (x), reverse = True):
            if not filters:
                break
            for f in list (filters.keys ()):
                if f in model_fks:
                    _filters [model_fks [f][0]] = filters.pop (f)
                if not f.startswith (fk + '__'):
                    continue
                _filters [f.replace (fk + '__', self._fk [fk] + '__')] = filters.pop (f)

        filters and _filters.update (filters)
        return _filters
    ...
    def _join_select_fields (self):
        ...
        for fk, jtable in sorted (fks.items (), key = lambda x: len ( [0]), reverse = True):
            ...
            self._feed ['select'] = rx.sub (r'\1{}.\2'.format (jtable), self._feed ['select'])

        for fk, (column, model) in model_fields.items ():
            rx = re.compile (r"(^|[^_.a-z0-9'])(?:{})($|[^_a-z0-9])".format (fk), re.I)
            self._feed ['select'] = rx.sub (r'\1{}\2'.format (column), self._feed ['select'])
    ...
    def _join_related (self, jtype, fk_chain):
        ...
        return self._join (jtype, target_table, '', on = '{}.{} = {}.{}'.format (table, fk_column, target_table, target_pk))
