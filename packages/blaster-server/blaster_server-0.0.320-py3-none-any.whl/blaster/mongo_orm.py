...
class Model(object):
	...
	def update(self, _update_query, more_conditions=None, **kwargs):
		...
		for shard_key, shard in cls._secondary_shards_.items():
			...
			for attr in secondary_Model._pk_attrs:
				_secondary_pk[attr] = self._original_doc.get(attr)

			_secondary_collection = secondary_Model.get_collection(
				_secondary_pk[shard_key]
			)
			...
			if(_shard_key_changed):
				...
				#old values check will not overwrite
				for attr in shard.attributes:
		...
		**kwargs
		...
		def query_collection(_Model, _collection, _query, projection, sort, offset, limit):
			...
			if(_Model._is_secondary_shard and not _no_requery):
				...
				ret = map(
					lambda x: cls.get(cls.pk_from_doc(x)),
					ret
				)
			else:
	...
	def commit(self, force=False):
		...
		if(self.__is_new): # try inserting
			...
			try:
				...
				self._id = self._insert_result.inserted_id
				committed = True
	...
	def delete(self):
		...
		for _shard_key, _seconday_shard in _Model._secondary_shards_.items():
			_pk = {}
			for k in _seconday_shard._Model_._pk_attrs.keys():
				_pk[k] = getattr(self, k)

			collection_shard = _seconday_shard._Model_.get_collection(
				str(getattr(self, _shard_key))
			)
			collection_shard.delete_one(_pk)


		#find which pimary shard it belongs to
		_Model.get_collection(
...
class Connection:
	...
	def inititalize(self, Model):
		...
		_last_pk_is_unique_index = False
		for _index in _model_indexes:
			...
			if(	Model._shard_key_ == _index_shard_key
				and (
						not Model._pk_attrs
						or (is_unique_index and not _last_pk_is_unique_index)
				)
			):
				_last_pk_is_unique_index = is_unique_index
